...
describe("Chapter 7 · The Brass Embassy",()=>{
 ...
 it("gives every new enemy working AI, skills and unique portrait art",()=>{for(const id of enemyIds){const enemy=ENEMIES[id];expect(enemy,id).toBeTruthy();expect(ENEMY_BEHAVIORS[enemy!.behaviorId],enemy!.behaviorId).toBeTruthy();expect(ENEMY_PORTRAITS[id]).toMatchObject({atlasId:"ironHillsChapter7"});for(const skillId of enemy!.skillIds){expect(ENEMY_SKILLS[skillId],skillId).toBeTruthy();expect(getSkillIconArt(skillId).atlas).toBe("ironHillsChapter7");}}});

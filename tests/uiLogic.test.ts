...
describe("Milestone 5 UI state selectors", () => {
  ...
  it("derives notifications from live guild state", () => { const guild = createGuild(); expect(getGuildNotifications(guild).some((item) => item.id === "contracts")).toBe(true); const injured = { ...testHero(), conditions: [{ conditionId: "injured" as const, remainingDuration: 2 }] }; expect(getGuildNotifications({ ...guild, heroes: [injured] }).some((item) => item.id === "injured")).toBe(true); });
});

...
describe("recruitment finances and scouting", () => {
  it("calculates the mandatory fee and contract examples", () => { expect(calculateRecruitmentFee(4,80,-.20)).toBe(656); expect(calculateContractCosts(700,100,24)).toEqual({contractSalary:2400,totalEstimatedCost:3100}); });
  ...
  it("narrows attributes, fee, and salary demand through all scouting levels", () => { const generated=generateRecruitmentCandidate(createSeededRandom(1),1,0,"standard"); let candidate=generated; const initialFeeWidth=candidate.recruitmentFeeEstimateMax-candidate.recruitmentFeeEstimateMin; const strength = candidate.heroPreview.baseAttributes.strength; expect(candidate.attributeEstimates.strength).toEqual({ minimum: Math.max(3, strength - 3), maximum: Math.min(20, strength + 3) }); candidate=scoutCandidate(candidate); expect(candidate.attributeEstimates.strength).toEqual({ minimum: Math.max(3, strength - 2), maximum: Math.min(20, strength + 2) }); expect(candidate.recruitmentFeeEstimateMax-candidate.recruitmentFeeEstimateMin).toBeLessThan(initialFeeWidth); candidate=scoutCandidate(candidate); expect(candidate.attributeEstimates.strength).toEqual({ minimum: Math.max(3, strength - 1), maximum: Math.min(20, strength + 1) }); candidate=scoutCandidate(candidate); expect(candidate.attributeEstimates.strength).toEqual({ minimum: strength, maximum: strength }); expect([candidate.recruitmentFeeEstimateMin,candidate.recruitmentFeeEstimateMax]).toEqual([candidate.recruitmentFee,candidate.recruitmentFee]); expect([candidate.weeklySalaryEstimateMin,candidate.weeklySalaryEstimateMax]).toEqual([candidate.weeklySalary,candidate.weeklySalary]); });
...
describe("regional scout expeditions", () => {
  ...
  it.each(["human", "elf", "dwarf", "orc"] as const)("takes three days before returning five narrowed %s candidates", (raceId) => {
    ...
    expect(result.recruitment.candidates.every((candidate) => candidate.scoutingLevel === 1 && candidate.attributeEstimates.strength.maximum - candidate.attributeEstimates.strength.minimum <= 4)).toBe(true);

...
export function GuildmasterSkillTreeScreen({ onBack }: { onBack(): void }) {
  ...
  return <ScrollView contentContainerStyle={styles.content}>
    ...
    <TreeBranch title="SCOUTING NETWORK" subtitle="Candidate knowledge, regional searches and dispatch control" tiers={SCOUTING} profile={profile} selectedId={selectedId} select={selectNode} />
    <TreeBranch title="ARTISAN DISTRICT" subtitle="Construction charters and workshop mastery" tiers={ARTISANS} profile={profile} selectedId={selectedId} select={selectNode} />
    <TreeBranch title="GUILD LOGISTICS" subtitle="Travel supplies, tavern revenue and faster material expeditions" tiers={LOGISTICS} profile={profile} selectedId={selectedId} select={selectNode} />
    <SkillDetailsModal visible={detailsOpen} onClose={() => setDetailsOpen(false)}><SkillDetails profile={profile} selectedId={selectedId} onLearn={learn} message={message} /></SkillDetailsModal>
  </ScrollView>;
}

function TreeBranch({ title, subtitle, tiers, profile, selectedId, select }: { title: string; subtitle: string; tiers: GuildmasterSkillId[][]; profile: GuildmasterProfile; selectedId: GuildmasterSkillId; select(id: GuildmasterSkillId): void }) {
...
function GuildNode({ id, state, selected, select }: { id: GuildmasterSkillId; state: ReturnType<typeof nodeState>; selected: boolean; select(id: GuildmasterSkillId): void }) { const skill = GUILDMASTER_SKILLS[id]; return <View style={styles.nodeGroup}><Pressable accessibilityLabel={`${skill.name}, ${state.replace("_", " ")}`} accessibilityRole="button" accessibilityState={{ selected }} onPress={() => select(id)} style={[styles.node, state === "unlocked" && styles.nodeUnlocked, state === "available" && styles.nodeAvailable, (state === "locked" || state === "no_points") && styles.nodeLocked, selected && styles.nodeSelected]}><GameIcon id={SKILL_ICONS[id]} size={49} />{state === "unlocked" && <Text style={styles.check}>✓</Text>}{state === "locked" && <View style={styles.lockOverlay}><Text style={styles.lock}>◆</Text></View>}</Pressable><Text style={styles.nodeName}>{skill.name}</Text><Text style={styles.nodeState}>{state === "locked" ? `LV ${skill.levelRequirement} · LOCKED` : state === "no_points" ? "NO POINTS" : state.toUpperCase()}</Text></View>; }
...
const styles = StyleSheet.create({
  nodeGroup: { alignItems: "center", width: 76 }, nodeName: { color: colors.text, fontSize: 11, fontWeight: "800", lineHeight: 14, textAlign: "center", marginTop: 6 }, nodeState: { color: colors.muted, fontSize: 9, lineHeight: 12, textAlign: "center", marginTop: 2 },

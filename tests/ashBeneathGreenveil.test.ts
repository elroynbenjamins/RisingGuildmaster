...
describe("Ash Beneath Greenveil", () => {
  ...
  it("uses dedicated portraits and skill icons for the story enemies", () => {
    expect(new Set(ENEMY_IDS.map((id) => ENEMY_PORTRAITS[id])).size).toBe(ENEMY_IDS.length);
    expect(ENEMY_IDS.every((id) => Boolean(ENEMY_PORTRAITS[id]))).toBe(true);
    ["ember_bite", "cinder_scuttle", "ember_fed", "cinder_blade", "ash_bomb", "scale_hammer", "wardfire_pulse", "ancient_scale_shell", "sleeping_ember"].forEach((skillId) => expect(GENERATED_SKILL_ICON_ART[skillId]).toBeDefined());
  });

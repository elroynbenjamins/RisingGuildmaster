...
export function scoutCandidate(candidate: RecruitmentCandidate): RecruitmentCandidate {
  if (candidate.scoutingLevel >= 3) throw new Error("Candidate is already fully scouted");
  const level = (candidate.scoutingLevel + 1) as ScoutingLevel;
  const fee = financialEstimate(candidate.recruitmentFee, FINANCIAL_RADII[level]);
  const salary = financialEstimate(candidate.weeklySalary, FINANCIAL_RADII[level]);
  return {
    ...candidate,
    scoutingLevel: level,
    attributeEstimates: attributeEstimates(candidate.heroPreview.baseAttributes, level),
    recruitmentFeeEstimateMin: fee.minimum,
    recruitmentFeeEstimateMax: fee.maximum,
    weeklySalaryEstimateMin: salary.minimum,
    weeklySalaryEstimateMax: salary.maximum,
  };
}

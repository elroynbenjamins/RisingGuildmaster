...
import { filterQuests, getQuestReadiness, sortQuestsForRoster } from "../src/ui/questList";
...
describe("Milestone 5 UI state selectors", () => {
  it("assigns a distinct name color to every playable race", () => { expect(new Set(Object.values(RACE_NAME_COLORS)).size).toBe(7); expect(getRaceNameColor("human")).toBe(RACE_NAME_COLORS.human); expect(Object.values(RACE_NAME_COLORS).every((color) => /^#[0-9A-F]{6}$/i.test(color))).toBe(true); });
  ...
  it("marks and sorts quests by practical roster readiness", () => { expect(getQuestReadiness(QUESTS.troll_hunt!, 4)).toBe("DANGER"); expect(getQuestReadiness(QUESTS.troll_hunt!, 5)).toBe("READY"); expect(sortQuestsForRoster([QUESTS.troll_hunt!, QUESTS.orchard_road_patrol!], 2)[0]?.id).toBe("orchard_road_patrol"); });
  it("filters inventory categories and shows tactical equipment differences", () => { expect(filterInventory(["worn-sword", "padded-armor"], "Weapons").map((item) => item.id)).toEqual(["worn-sword"]); const hero = { ...testHero(), classId: "warrior" as const, equipment: { ...testHero().equipment, weapon: null } }; const rows = compareEquipment(hero, "worn-sword"); expect(rows.some((row) => row.label === "STR")).toBe(false); expect(hero.equipment.weapon).toBeNull(); });

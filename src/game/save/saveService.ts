...
function deserializeGuildData(value: string): GuildState {
  const saved = JSON.parse(value) as Omit<GuildState,"uiPreferences">&{uiPreferences?:Partial<UiPreferences>}; const recruitment = saved.recruitment ?? createRecruitmentState(saved.currentDay ?? 1); const currentDay = saved.currentDay ?? 1;

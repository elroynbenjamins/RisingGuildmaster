...
export function getPostBattleManagementActions(summary: PostBattleSummary, guild: GuildState): PostBattleManagementAction[] {
  ...
  if (needsRecovery) actions.push({ id: "recovery", title: "Restore the Party", description: "HP loss, injuries, and fallen heroes persist after battle. The Temple uses gold for treatment; reviving a fallen hero also requires a soul gem.", actionLabel: "Open Temple", isNew: guild.world.worldFlags[SEEN_FLAGS.recovery] !== true });

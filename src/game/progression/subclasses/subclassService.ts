...
export function getHeroSkillIds(hero: Hero): string[] { return [CLASS_SKILL_TREES[hero.classId].basicSkillId, ...getLearnedClassSkillIds(hero), ...(hero.subclassId ? SUBCLASSES[hero.subclassId]?.addedSkillIds ?? [] : [])]; }
export function getSubclassModifierValue(hero: Hero, target: ModifierTarget, operation: "flat" | "percentage", hpRatio = 1): number { return hero.subclassId ? (SUBCLASSES[hero.subclassId]?.modifiers ?? []).filter((modifier) => modifier.target === target && modifier.operation === operation && (!modifier.condition || hpRatio <= modifier.condition.value)).reduce((sum, modifier) => sum + modifier.value, 0) : 0; }

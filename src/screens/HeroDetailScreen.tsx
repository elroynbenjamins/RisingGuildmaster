...
export function HeroDetailScreen({
  ...
  return <ScrollView style={{ backgroundColor: c.background }} contentContainerStyle={styles.content}>
    ...
    {tab === "Story" && <>
      ...
      <Panel>{liveHero.traitIds.map((id) => { const trait = TRAITS[id]; const traitColor = trait.polarity === "positive" ? c.green : trait.polarity === "negative" ? c.danger : c.gold; return <View key={id} style={styles.entry}><Text style={[styles.entryName, { color: traitColor }]}>{trait.name} · {trait.category.toUpperCase()}</Text><Text style={[styles.hint, { color: c.muted }]}>{trait.description}</Text>{trait.modifiers.map((modifier, index) => <Text key={index} style={[styles.traitModifier, { color: traitColor }]}>{modifierText(modifier.operation, modifier.value, modifier.target)}{modifier.condition ? " while at or below 50% HP" : ""}</Text>)}</View>; })}</Panel>

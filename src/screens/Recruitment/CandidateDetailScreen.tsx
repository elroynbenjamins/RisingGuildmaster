...
import { scoutingCost } from "../../game/recruitment/scoutingService";
...
export function CandidateDetailScreen({ candidateId, onBack }: { candidateId: string; onBack(): void }) {
  ...
  useEffect(() => {
    if (!guild.tutorial.active || (guild.tutorial.step !== "inspect_candidate" && guild.tutorial.step !== "recruit_first")) { tutorialPulse.setValue(1); return; }
    const pulse = Animated.loop(Animated.sequence([
      Animated.timing(tutorialPulse, { toValue: .45, duration: 650, useNativeDriver: true }),
      Animated.timing(tutorialPulse, { toValue: 1, duration: 650, useNativeDriver: true }),
    ]));
    pulse.start();
    return () => pulse.stop();
  }, [guild.tutorial.active, guild.tutorial.step, tutorialPulse]);
  ...
  const run = (action: () => string | null, success: string) => { const error = action(); if(error){triggerTactileFeedback(guild.uiPreferences.tactileFeedback,"warning");showToast({title:"Candidate Action Failed",message:error,tone:"danger"});return;}triggerTactileFeedback(guild.uiPreferences.tactileFeedback,"confirm");showToast({title:success.includes("reserved")?"Candidate Reserved":"Scouting Updated",message:success,tone:"success"}); };
  const confirm = () => showDialog({ title: `Sign ${hero.name}?`, message: `${presentation.roleLabel} · ${presentation.fitLabel}\nEstimated fee: ${range(candidate.recruitmentFeeEstimateMin, candidate.recruitmentFeeEstimateMax)} gold\nSalary demand: ${range(candidate.weeklySalaryEstimateMin, candidate.weeklySalaryEstimateMax)} gold/week\nProjected payroll: ${range(presentation.payrollAfterMin, presentation.payrollAfterMax)} gold/week\nContract: ${candidate.contractLengthWeeks} weeks\nEstimated total contract cost: ${range(totalMin, totalMax)} gold`, eyebrow: "GUILD CONTRACT", actions: [{ label: "Keep Looking", tone: "secondary" }, { label: "Sign Contract", tone: "primary", onPress: () => { const error = recruitCandidate(candidateId); if (error) { triggerTactileFeedback(guild.uiPreferences.tactileFeedback,"warning"); showToast({title:"Contract Failed",message:error,tone:"danger"}); } else { triggerTactileFeedback(guild.uiPreferences.tactileFeedback,"confirm"); showToast({title:"Contract Signed",message:`${hero.name} joined the guild.`,tone:"success"}); onBack(); } } }] });
...
const styles = StyleSheet.create({
  ...
  offerGrid:{flexDirection:"row",flexWrap:"wrap",gap:8}, offerCell:{backgroundColor:"transparent",flexBasis:"45%",flexGrow:1,minWidth:100,padding:4}, offerLabel:{color:colors.muted,fontSize:10,fontWeight:"500",letterSpacing:0}, offerValue:{color:colors.text,fontSize:13,fontWeight:"900",marginTop:3},
});

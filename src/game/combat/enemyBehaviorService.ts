...
export function selectEnemySkill(definition: EnemyDefinition, instance: EnemyInstance, actor: CombatUnit, heroes: readonly CombatUnit[], enemies: readonly CombatUnit[], board?: CombatBoardState): CombatSkillDefinition {
  ...
  for (const rule of behavior.rules) {
    ...
    if (targets.length && conditionsMatch(rule.conditions, actor, targets, livingEnemies, livingHeroes)) candidates.push({ skill, score: rule.priority * 3 + scoreEnemySkillTargets(skill, actor, targets) });
  }
  ...
  if (basicTargets.length) candidates.push({ skill: basic, score: scoreEnemySkillTargets(basic, actor, basicTargets) });

...
export interface DungeonRunModifierDefinition { id: string; name: string; description: string; enemyModifiers: Modifier[]; rewardGoldModifier: number; rareLootModifier: number; healingPowerModifier: number; enemyInitiativeModifier?: number; enemyMovementRangeModifier?: number; scoreBonus: number }
...
export type DungeonRunGrade = "S" | "A" | "B" | "C" | "D";
export interface DungeonRunScore { total: number; roomScore: number; dangerScore: number; survivalScore: number; victoryScore: number; grade: DungeonRunGrade }

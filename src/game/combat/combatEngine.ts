import { getSideQuestDifficultyLabel, getSideQuestEnemyRollModifiers } from "../quests/sideQuestDifficultyService";
import { resolveBossPhaseTransitions } from "../bosses/bossCombatMechanicService";
...
import type { EncounterObjectiveDefinition } from "../quests/questTypes";
...
import { validateEncounterReachability } from "./grid/encounterReachability";
...
export function createCombatState(questId: string, encounterIndex: number, heroes: readonly Hero[], random: RandomSource, carried?: readonly HeroCombatInstance[], setup?: QuestCombatSetup, relationships: readonly HeroRelationship[] = [], difficultyId: GameDifficultyId = "standard", enemyLevelModifier = 0): CombatState {
  ...
  const objective = getEncounterObjective(encounter);
  ...
  const sideQuestDifficultyLabel = getSideQuestDifficultyLabel(quest);
  ...
  const sideQuestModifiers = getSideQuestEnemyRollModifiers(quest);
  const setupLabel = [setup?.label, getSideQuestDifficultyLabel(quest)].filter(Boolean).join(" · ");
...
export function beginCombat(state: CombatState, random: RandomSource, automaticTurnLimit = Number.POSITIVE_INFINITY): CombatState {
  ...
  return advanceCombat(resolveBossPhaseTransitions(initializeRaidMechanics({ ...state, combatStarted: true })), random, automaticTurnLimit);
}
...
function withOutcome(state: CombatState): CombatState {
  ...
  const objectiveComplete = isEncounterObjectiveComplete(
    state.objective,
    state.round,
  ...
  if (objectiveComplete) return { ...state, status: "victory", awaitingHeroId: null };
  return state;
}
...
export function advanceCombat(state: CombatState, random: RandomSource, automaticTurnLimit = Number.POSITIVE_INFINITY): CombatState {
  ...
  while (next.status === "active" && !next.awaitingHeroId && safety++ < 100) {
    if (next.turnCursor >= next.turnOrderIds.length) {
      next = resolveBossPhaseTransitions(withOutcome(resolveRaidRoundStart({ ...next, round: next.round + 1, turnCursor: 0, spentReactionIds: [] })));
    }
    ...
    automaticTurnsResolved += 1;
    next = withOutcome(resolveBossPhaseTransitions({ ...next, board, enemies, heroes, turnCursor: next.turnCursor + 1, turn: next.turn + 1 }));
  }
  return next;
}
...
export function moveCurrentHero(state: CombatState, destination: GridPosition, random: RandomSource, automaticTurnLimit = Number.POSITIVE_INFINITY): CombatState {
  ...
  const result = withOutcome({ ...moved.state, actions: { ...state.actions, movementUsed: true }, log: [...moved.state.log, { turn: state.turn, actorId: combatant.hero.id, actionId: "move", targetIds: [], message: `${combatant.hero.name} moved ${moved.travelledTiles} tiles${moved.state.heroes[index]?.unit.isAlive ? "." : " before being defeated."}` }],lastVisualEvent });
...
export function performHeroTurn(state: CombatState, skillId: string, random: RandomSource, selectedTargetId?: string, targetPosition?: GridPosition): CombatState {
  ...
  return withOutcome(resolveBossPhaseTransitions({ ...logged, actions: { ...state.actions, combatActionUsed: true, usedSkillId: skillId } }));
}

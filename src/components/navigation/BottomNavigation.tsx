...
export function BottomNavigation({ active, onSelect }: { active: MainTab; onSelect(tab: MainTab): void }) { return <View style={styles.bar}>{MAIN_TABS.map((tab) => <Pressable key={tab} accessibilityRole="tab" accessibilityState={{ selected: active === tab }} onPress={() => onSelect(tab)} style={[styles.tab, active === tab && styles.selected]}><GameIcon id={ICONS[tab]} size={27} framed={false} /><Text style={[styles.label, active === tab && styles.active]}>{tab}</Text></Pressable>)}</View>; }

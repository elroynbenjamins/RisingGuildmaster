...
export function findShortestPath(board: CombatBoardState, start: GridPosition, destination: GridPosition, maxMovement = Infinity): GridPosition[] | null {
  ...
  while (queue.length) {
    ...
    for (const neighbor of orthogonalNeighbors(current.position)) {
      ...
      const cost = current.cost + tile.movementCost; const key = positionKey(neighbor); if (cost > maxMovement || cost >= (bestCost.get(key) ?? Infinity)) continue;
...
export function getReachablePositions(board: CombatBoardState, start: GridPosition, movementRange: number): GridPosition[] { return board.tiles.filter((tile) => !tile.blocksMovement && !tile.occupantId && findShortestPath(board, start, tile.position, movementRange)).map((tile) => tile.position); }

  ...
  return <View accessibilityLabel="Party health and resources" style={styles.list}>
    {heroes.map(({ hero, instance }) => {
      ...
      return <View key={hero.id} style={[styles.member,{backgroundColor:themeColors.panel2,borderColor:themeColors.border}, compact && styles.compactMember, active && {borderColor:themeColors.gold,borderWidth:2}, !instance.isAlive && styles.defeated]}>
        ...
        <ResourceBar label="HP" value={instance.currentHP} maximum={instance.maxHP} color={getCombatHealthState(instance.currentHP,instance.maxHP)==="critical"?themeColors.danger:getCombatHealthState(instance.currentHP,instance.maxHP)==="wounded"?themeColors.gold:themeColors.green} />
        {instance.isAlive && getCombatHealthState(instance.currentHP, instance.maxHP) === "critical" && <Text style={[styles.criticalWarning,{color:themeColors.danger}]}>! CRITICAL HP · Protect or heal this hero</Text>}
...
const styles = StyleSheet.create({
  ...
  criticalWarning: { fontSize: 9, fontWeight: "900", marginTop: 4 },

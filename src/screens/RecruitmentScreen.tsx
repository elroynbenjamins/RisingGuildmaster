  ...
  return <ScrollView ref={scrollRef} contentContainerStyle={styles.content}>
    ...
    {displayedCandidates.map((candidate) => {
      ...
      return <Panel key={candidate.candidateId} style={[styles.card, { borderColor: cardBorder }]}>
        ...
        <Text style={styles.fitDetail}>Scouted potential · {range(candidate.potentialEstimateMin, candidate.potentialEstimateMax)}</Text>
        <Text style={styles.fitDetail}>{presentation.fitDetail}</Text>
        {getRecruitmentRecommendation(guild, candidate) && <Text style={styles.fitDetail}>Recommended by {getRecruitmentRecommendation(guild, candidate)!.heroName}: {getRecruitmentRecommendation(guild, candidate)!.reason}.</Text>}
        <Text style={styles.traits}>TRAITS · {hero.traitIds.map((id) => TRAITS[id].name).join(" · ")}</Text>
        <View style={styles.economyLine}><StatusChip label={presentation.economyLabel} tone={presentation.economyTone} /><Text style={styles.economyText}>After min. fee: {presentation.goldAfterMinimumFee} G · Payroll after hire: {range(presentation.payrollAfterMin, presentation.payrollAfterMax)} G/week · {candidate.contractLengthWeeks} weeks</Text></View>
...
const styles = StyleSheet.create({ cost: {color: colors.text, fontWeight: "800", marginTop: 5},formerHeading:{color:colors.gold,fontSize:10,fontWeight:"900",letterSpacing:1,marginBottom:4,marginTop:4},formerIntro:{color:colors.muted,fontSize:10,lineHeight:15,marginBottom:8},formerCard:{borderColor:"#705b3e",gap:8,marginBottom:8},formerCooling:{opacity:.64},retiredCard:{borderColor:"#7e7459"},retiredMeta:{color:colors.gold},formerMeta:{color:colors.green,fontSize:9,fontWeight:"900",marginTop:4},
  ...
  fitDetail: { color: colors.muted, fontSize: 10, lineHeight: 15, marginTop: 9 },

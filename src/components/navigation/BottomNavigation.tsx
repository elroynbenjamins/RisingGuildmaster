...
export function BottomNavigation({ active, onSelect }: { active: MainTab; onSelect(tab: MainTab): void }) { const{colors:c}=useTheme();const notices=useActionNotifications();return <View style={[styles.bar,{backgroundColor:c.panel,borderColor:c.border}]}>{MAIN_TABS.map((tab) => <Pressable key={tab} accessibilityRole="tab" accessibilityState={{ selected: active === tab }} onPress={() => onSelect(tab)} style={[styles.tab, active === tab && styles.selected]}>{(tab === "Guild" && notices.manage || tab === "Heroes" && notices.heroes) ? <View style={{ position: "absolute", top: 0, right: "25%" }}><NotificationDot/></View> : null}<GameIcon id={ICONS[tab]} size={27} framed={false} /><Text style={[styles.label,{color:c.muted},active === tab && {color:c.gold}]}>{tab}</Text></Pressable>)}</View>; }

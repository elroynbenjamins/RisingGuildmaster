...
import { advanceCombat, createCombatState, endCurrentHeroTurn, moveCurrentHero, performHeroTurn } from "../game/combat/combatEngine";
import type { HeroCombatInstance } from "../game/combat/combatTypes";
...
export function CombatScreen({ questId, heroes, onQuestEnd }: { questId: string; heroes: Hero[]; onQuestEnd(status: "victory" | "defeat", instances: HeroCombatInstance[]): void }) {
  const random = useRef(createSeededRandom(randomSeed())); const quest = QUESTS[questId]!;
  const [state, setState] = useState(() => advanceCombat(createCombatState(questId, 0, heroes, random.current), random.current));
  ...
  const current = state.heroes.find((item) => item.hero.id === state.awaitingHeroId); const selectedSkill = selectedSkillId ? HERO_SKILLS[selectedSkillId] : undefined; const battlefield = BATTLEFIELDS[state.board.environmentId];
  const units = [...state.heroes.map((item) => item.unit), ...state.enemies.map((item) => item.unit)];
  const labels = useMemo(() => Object.fromEntries([...state.heroes.map((item) => [item.unit.combatantId, CLASSES[item.hero.classId].name]), ...state.enemies.map((item) => [item.unit.combatantId, getEnemyDefinition(item.instance.enemyDefinitionId).name])]), [state.heroes, state.enemies]);
  const reachableKeys = useMemo(() => new Set(current && mode === "move" && !state.actions.movementUsed ? getReachablePositions(state.board, current.unit.position, current.unit.movementRange).map(positionKey) : []), [current, mode, state.actions.movementUsed, state.board]);
  ...
  const resetSelection = () => { setMode(null); setSelectedSkillId(undefined); setSelectedPosition(undefined); setSelectedTargetId(undefined); };
  const onTilePress = (position: GridPosition, occupantId: string | null) => {
    try { setError(null);
      ...
      if (mode === "skill" && targetableKeys.has(positionKey(position))) { setSelectedPosition(position); setSelectedTargetId(occupantId ?? undefined); }
  ...
  const continueQuest = () => { const carried = state.heroes.map((item) => recoverBetweenEncounters(item.instance)); setState(advanceCombat(createCombatState(questId, state.encounterIndex + 1, heroes, random.current, carried), random.current)); resetSelection(); };
  const lastEncounter = state.encounterIndex >= quest.encounterIds.length - 1;
  ...
  return <ScrollView contentContainerStyle={styles.content}><Text style={styles.title}>{quest.name}</Text><Text style={styles.encounter}>Encounter {state.encounterIndex + 1} / {quest.encounterIds.length} · Round {state.round} · Turn {state.turn}</Text>
    <Text style={styles.encounter}>{battlefield?.name ?? "Battlefield"} · {state.board.width}×{state.board.height}</Text><Text style={styles.detail}>{Object.values(battlefield?.legend ?? {}).join("  ·  ")}</Text><CombatBoard board={state.board} units={units} labels={labels} reachableKeys={reachableKeys} targetableKeys={targetableKeys} affectedKeys={affectedKeys} selectedUnitId={current?.unit.combatantId} selectedPosition={selectedPosition} onTilePress={onTilePress} />
    {state.lastRoll?.diceRoll !== undefined && <DiceRollDisplay diceType={20} roll={state.lastRoll.diceRoll} modifier={(state.lastRoll.attackTotal ?? 0) - state.lastRoll.diceRoll} total={state.lastRoll.attackTotal ?? 0} targetValue={state.lastRoll.targetValue ?? 0} result={state.lastRoll.rollResult ?? "miss"} />}
    {state.status === "active" && current && <><SectionTitle>CURRENT TURN · {current.hero.name}</SectionTitle><CombatantPanel name={`${current.hero.name} · ${CLASSES[current.hero.classId].name}`} currentHP={current.instance.currentHP} maxHP={current.instance.maxHP} resource={`Mana ${current.instance.currentMana}/${current.instance.maxMana} · Stamina ${current.instance.currentStamina}/${current.instance.maxStamina}`} />
      <View style={styles.actions}><Pressable disabled={state.actions.movementUsed} onPress={() => { setMode("move"); setSelectedSkillId(undefined); setSelectedPosition(undefined); }} style={[styles.modeButton, mode === "move" && styles.active, state.actions.movementUsed && styles.disabled]}><Text style={styles.modeText}>Move ({current.unit.movementRange})</Text></Pressable><Pressable onPress={endTurn} style={styles.endButton}><Text style={styles.modeText}>End Turn</Text></Pressable></View>
      <SectionTitle>SKILLS</SectionTitle><SkillBar items={skillItems} selectedId={selectedSkillId} onSelect={(id) => { setMode("skill"); setSelectedSkillId(id); setSelectedPosition(undefined); setSelectedTargetId(undefined); }} />
      {selectedPosition && selectedSkillId && <ActionButton label={`Confirm ${HERO_SKILLS[selectedSkillId]!.name}`} onPress={confirmSkill} />}
      <Text style={styles.hint}>{mode === "move" ? "Tap a teal bordered reachable tile." : mode === "skill" ? "Tap a dashed target tile, review the preview, then confirm." : "Choose Move, a skill, or End Turn."}</Text></>}

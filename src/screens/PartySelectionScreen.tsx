...
export function PartySelectionScreen({ questId, onBack, start }: { questId: string; onBack(): void; start(party: Party): void }) {
  const { guild } = useGuild(); const quest = QUESTS[questId]!; const [ids, setIds] = useState<string[]>([]); const staminaCost = getQuestAdventureStaminaCost(quest);
  ...
  const averageLevel = selectedHeroes.length ? selectedHeroes.reduce((sum, hero) => sum + hero.level, 0) / selectedHeroes.length : 0; const recommendedMin = quest.recommendedLevelMin ?? 1; const underLevelled = averageLevel > 0 && averageLevel < recommendedMin;
  ...
  return <ScrollView contentContainerStyle={styles.content}><BackButton onPress={onBack} /><Text style={styles.title}>Assemble Party</Text><Text style={styles.quest}>{quest.name} · Difficulty {quest.difficulty} · Recommended Lv {recommendedMin}–{quest.recommendedLevelMax ?? recommendedMin} · Cost {staminaCost} readiness</Text>
    ...
    {selectedHeroes.length > 0 && <Panel style={[styles.readiness, underLevelled && styles.dangerPanel]}><Text style={styles.readinessTitle}>Party average level: {averageLevel.toFixed(1)}</Text><Text style={underLevelled ? styles.danger : styles.ready}>{underLevelled ? `DANGER: Below recommended level ${recommendedMin}.` : "Party meets the recommended level."}</Text></Panel>}

...
function Game() {
  ...
  if (route.name === "questDetail") return <QuestDetailScreen questId={route.questId} onBack={() => main("Quests")} assemble={() => setRoute({ name: "questBriefing", questId: route.questId, back: "quest" })} />;
  ...
  else if (tab === "Quests") screen = <QuestSelectionScreen selectQuest={(questId) => setRoute({ name: "questDetail", questId })} openCampaign={() => setRoute({ name: "campaign" })} openDungeons={() => setRoute({ name: "dungeon" })} openOperations={() => setRoute({ name: "operations" })} />;
...
export default function App() { return <SafeAreaProvider><SafeAreaView style={styles.safe} edges={["top", "right", "bottom", "left"]}><GameDialogProvider><GuildProvider><StatusBar barStyle="light-content" backgroundColor={colors.background} /><Game /></GuildProvider></GameDialogProvider></SafeAreaView></SafeAreaProvider>; }

...
export function CombatScreen({ questId, heroes, combatSetup, initialHeroInstances, onQuestEnd, onEnemiesEncountered, onExit }: { questId: string; heroes: Hero[]; combatSetup?: QuestCombatSetup; initialHeroInstances?: HeroCombatInstance[]; onQuestEnd(status: "victory" | "defeat", instances: HeroCombatInstance[]): void; onEnemiesEncountered?(enemyDefinitionIds: string[]): void; onExit?(): void }) {
  ...
  useEffect(() => {
    ...
    const delay = actorSide === "enemies" ? ENEMY_TURN_DELAY_MS : 0;
  ...
  }, [state.combatStarted, state.status, state.awaitingHeroId, state.turnCursor, state.round]);
  ...
  const movementPath = useMemo(() => current && mode !== "skill" && selectedPosition && reachableKeys.has(positionKey(selectedPosition)) ? findShortestPath(state.board, current.unit.position, selectedPosition, currentMovementRange, current.unit.ignoredTerrainMovementCosts) ?? [] : [], [current, mode, selectedPosition, reachableKeys, state.board, currentMovementRange]);
  ...
  return <ScrollView contentContainerStyle={styles.content}><View style={styles.titleRow}><Text style={styles.title}>{quest.name}</Text><Pressable onPress={() => { const leave = onExit ?? (() => onQuestEnd("defeat", state.heroes.map((item) => item.instance))); state.status === "active" ? showDialog({ title: "Leave combat?", message: "Current encounter progress will be lost.", eyebrow: "RETREAT WARNING", tone: "danger", actions: [{ label: "Stay", tone: "secondary" }, { label: "Leave", tone: "danger", onPress: leave }] }) : leave(); }}><Text style={styles.exit}>Exit</Text></Pressable></View><Text style={styles.encounter}>Encounter {state.encounterIndex + 1} / {quest.encounterIds.length} · Round {state.round} · Turn {state.turn}</Text><TurnOrder ids={state.turnOrderIds} cursor={state.turnCursor} labels={labels} labelColors={labelColors} aliveIds={aliveIds} />
    ...
    {state.status === "active" && current && showTacticalDetails && <><SectionTitle>CURRENT TURN · {current.hero.name}</SectionTitle><CombatantPanel name={`${current.hero.name} · ${CLASSES[current.hero.classId].name}`} nameColor={getRaceNameColor(current.hero.raceId)} currentHP={current.instance.currentHP} maxHP={current.instance.maxHP} resource={`Mana ${current.instance.currentMana}/${current.instance.maxMana} · Stamina ${current.instance.currentStamina}/${current.instance.maxStamina}`} physicalDamage={current.unit.stats.physicalDamage} physicalDefense={current.unit.stats.physicalDefense} magicDamage={current.unit.stats.magicDamage} magicDefense={current.unit.stats.magicDefense} />
      ...
      <Text style={styles.hint}>{mode === "skill" ? "Tap a highlighted target to preview or use the skill. Double tap any empty teal tile to move first." : state.actions.movementUsed ? "Movement used. Choose a skill or end the turn." : "Teal tiles are always reachable. Double tap one to move, or choose a skill."}</Text></>}

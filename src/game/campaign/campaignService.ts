...
export function getLatestCampaignChapter() {
  return Object.values(CAMPAIGN_CHAPTERS).sort((a, b) => b.chapterNumber - a.chapterNumber)[0]!;
}
...
export function isCampaignQuestUnlocked(questId: string, state: WorldState): boolean {
  ...
  return chapter?.chapterNumber === state.campaignChapter
    && !state.completedCampaignNodeIds.includes(node.id)
    && campaignPrerequisitesMet(node, state);
}
...
export function completeCampaignNode(state: WorldState, nodeId: string): CampaignCompletionResult {
  ...
  const completedCampaignNodeIds = [...state.completedCampaignNodeIds, nodeId]; const worldFlags = { ...state.worldFlags, ...node.setWorldFlags }; const unlockedRegionIds = [...new Set([...state.unlockedRegionIds, ...(node.unlockRegionIds ?? [])])]; const discoveredSettlementIds = [...new Set([...state.discoveredSettlementIds, ...(node.discoverSettlementIds ?? [])])]; const chapterComplete = nodeId === chapter.nodeIds[chapter.nodeIds.length - 1]; const completedQuestIds = node.questId ? [...new Set([...state.completedQuestIds, node.questId])] : state.completedQuestIds;
  return { worldState: { ...state, completedCampaignNodeIds, completedQuestIds, worldFlags, unlockedRegionIds, discoveredSettlementIds, campaignChapter: chapterComplete ? chapter.chapterNumber + 1 : state.campaignChapter }, goldReward: chapterComplete ? chapter.completionGoldReward ?? 0 : 0, guildReputationReward: chapterComplete ? chapter.completionReputationReward ?? 0 : 0 };
}

...
import type { CombatBoardState, GridPosition, TerrainType } from "./gridTypes";
...
export function moveOccupant(board: CombatBoardState, occupantId: string, from: GridPosition, destination: GridPosition, movementRange: number, ignoredTerrainMovementCosts: readonly TerrainType[] = []): { board: CombatBoardState; path: GridPosition[] } {
  const path = findShortestPath(board, from, destination, movementRange, ignoredTerrainMovementCosts); if (!path) throw new Error("Destination is not reachable");

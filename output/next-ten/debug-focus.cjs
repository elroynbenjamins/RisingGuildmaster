...
(async()=>{
 ...
 for(const name of names){
  ...
  await page.goto(`http://127.0.0.1:8099/?screen=${screen}&state=${action==='Large'?'rich':action==='Raid'?'raid':action==='Empty'?'empty':'normal'}&theme=${action==='HighContrast'?'high_contrast':'guild_dark'}&quest=${action?.startsWith('Quest=')?action.slice(6):''}`,{waitUntil:'load',timeout:180000});
  ...
  let actionError;
  if(clickText&&!action.startsWith('Quest=')&&!['Narrow','Empty','HighContrast','Large'].includes(action)){
  ...
  if((screen==='HeroesScreen'||screen==='InventoryScreen')&&action!=='Empty'){
    const label=screen==='HeroesScreen'?'Search hero roster':'Search inventory';
    const input=page.getByRole('textbox',{name:label,exact:true});
    await input.fill('no-match-xyz');await page.waitForTimeout(200);
    verify(await page.getByRole('button',{name:'Clear filters',exact:true}).isVisible(),'Empty search offers filter reset');
    console.log('FOCUS',await input.evaluate(e=>({outline:getComputedStyle(e).outline,border:getComputedStyle(e).border,shadow:getComputedStyle(e).boxShadow,html:e.outerHTML})));await screenshot('empty-search');
    await page.getByRole('button',{name:'Clear '+label.toLowerCase(),exact:true}).click();await page.waitForTimeout(200);
    verify(await input.inputValue()==='','Search clear button removes query');
    await input.fill('no-match-xyz');await page.waitForTimeout(200);await page.getByRole('button',{name:'Clear filters',exact:true}).click();await page.waitForTimeout(200);
    verify(await input.inputValue()==='','Filter reset clears search');verify(await page.getByRole('tab',{name:'All',exact:true}).getAttribute('aria-selected')==='true','Filter reset returns to All');
    if(screen==='InventoryScreen'){
      await page.getByRole('button',{name:'Show equipment upgrades',exact:true}).click();await page.waitForTimeout(200);
      verify(await page.getByRole('button',{name:'Filter upgrades',exact:true}).getAttribute('aria-pressed')==='true','Upgrade summary applies upgrade filter');await screenshot('upgrades');
      await page.getByRole('button',{name:'Filter upgrades',exact:true}).click();await page.waitForTimeout(200);
    }
  }
  if(screen==='RecruitmentScreen'){
    const fit=page.getByRole('button',{name:'Party role coverage',exact:true});await fit.click();await page.waitForTimeout(200);verify(await fit.getAttribute('aria-expanded')==='true','Party coverage expands');await fit.click();await page.waitForTimeout(200);
    const hire=page.getByRole('button',{name:'Roster full',exact:true});verify(await hire.count()===3,'Each blocked hire explains roster capacity');verify(await hire.first().isDisabled(),'Capacity-blocked hire is disabled');
  }
  if(screen==='QuestSelectionScreen'){
    const more=page.getByRole('button',{name:'Special activities',exact:true});await more.click();await page.waitForTimeout(200);verify(await more.getAttribute('aria-expanded')==='true','Special activities disclosure expands');verify((await page.locator('body').innerText()).includes('GUILD RAIDS'),'Activity choices are retained');await screenshot('activities');await more.click();await page.waitForTimeout(200);
  }
  if(action==='Large'){
    verify((await page.locator('body').innerText()).includes('1.2B'),'Large gold total is compact');verify(await page.getByLabel('Gold: 1,234,567,890',{exact:true}).count()>0,'Full resource amount stays accessible');
  }

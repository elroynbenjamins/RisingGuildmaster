...
export function QuestExplorationScreen({ questId, party, onBack, onComplete }: { questId: string; party: Party; onBack(): void; onComplete(combatSetup?: QuestCombatSetup): void }) {
  ...
  const partyHeroes = guild.heroes.filter((hero) => party.heroIds.includes(hero.id)); const checkingHero = result ? partyHeroes.find((hero) => hero.id === result.check.heroId) : undefined;

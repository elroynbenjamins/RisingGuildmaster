...
describe("Chapter 8 · The Black Tide",()=>{
 ...
 it("gives every Crownless enemy AI, skills, lore-ready art and a distinct portrait cell",()=>{const cells=new Set<string>();for(const id of enemyIds){const enemy=ENEMIES[id];expect(enemy,id).toBeTruthy();expect(ENEMY_BEHAVIORS[enemy!.behaviorId]).toBeTruthy();const portrait=ENEMY_PORTRAITS[id]!;expect(portrait.atlasId).toBe("westernSeaChapter8");cells.add(`${portrait.column}:${portrait.row}`);for(const skillId of enemy!.skillIds){expect(ENEMY_SKILLS[skillId],skillId).toBeTruthy();expect(getSkillIconArt(skillId).atlas).toBe("westernSeaChapter8");}}expect(cells.size).toBe(6);});

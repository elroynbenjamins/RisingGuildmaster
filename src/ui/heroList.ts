...
export type HeroFilter = "All" | "Available" | "Injured" | "Fallen";
...
export function filterAndSortHeroes(heroes: Hero[], filter: HeroFilter, sort: HeroSort): Hero[] {
  const filtered = heroes.filter((hero) => filter === "All" || (filter === "Available" ? hero.isAvailable && hero.currentHP > 0 : filter === "Injured" ? hero.currentHP > 0 && hasInjury(hero.conditions) : hero.currentHP <= 0));

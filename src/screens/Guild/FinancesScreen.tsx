...
import { advanceGuildTime, dailyTavernIncome, paySalaryArrears, previewNextGuildDay, totalSalaryArrears } from "../../game/economy/guildCalendarService";
import { nextTavernLevel, startTavernUpgrade, TAVERN_LEVELS } from "../../game/economy/tavernService";
import { cancelContractDeparture, contractDepartureDay, markContractForDeparture, renewalSalaryForLength, renewHeroContract } from "../../game/recruitment/contractService";
...
export function FinancesScreen({ onBack }: { onBack(): void }) {
  ...
  const renew = (heroId:string,weeks:RenewalLengthWeeks) => { try { updateGuild(renewHeroContract(guild,heroId,weeks)); } catch(error){ showDialog({title:"Contract renewal failed",message:error instanceof Error?error.message:"Renewal failed",tone:"danger"}); } };
  ...
  return <ScrollView contentContainerStyle={styles.content}>
    ...
    {guild.heroContracts.length ? guild.heroContracts.map((contract) => {
      const hero = guild.heroes.find((item) => item.id === contract.heroId); const owed = guild.finance.salaryArrearsByHeroId[contract.heroId] ?? 0; const days=Math.max(0,contract.endDay-guild.currentDay);
      return <Panel key={contract.heroId} style={[styles.contract,contract.status!=="active"&&styles.contractAttention]}><View style={styles.contractTop}><View style={styles.flex}><Text style={[styles.heroName, hero && { color: getRaceNameColor(hero.raceId) }]}>{hero?.name ?? "Unknown Hero"}</Text><Text style={styles.contractMeta}>{hero && hero.currentHP <= 0 ? "Salary paused · fallen" : contract.weeklySalary+" gold/week"} · {contract.status==="expired"?"grace until Day "+contractDepartureDay(contract):days+" days remaining"}</Text></View><View style={styles.contractRight}><Text style={contract.status === "expired" ? styles.danger : contract.status === "expiring" ? styles.warning : styles.active}>{contract.status.toUpperCase()}</Text>{owed > 0 && <Text style={styles.danger}>{owed} owed</Text>}</View></View>{hero && contract.status!=="active"?<View style={styles.renewals}>{([4,8,12] as RenewalLengthWeeks[]).map((weeks)=><SecondaryButton key={weeks} label={weeks+"w · "+renewalSalaryForLength(contract,hero.level,weeks)+"g/wk"} disabled={owed>0} onPress={()=>renew(hero.id,weeks)}/>) }{contract.renewalIntent==="depart"?<SecondaryButton label="Cancel Departure" onPress={()=>updateGuild(cancelContractDeparture(guild,contract.heroId))}/>:<SecondaryButton label="Let Contract Expire" onPress={()=>updateGuild(markContractForDeparture(guild,contract.heroId))}/>}</View>:null}</Panel>;
...
const styles = StyleSheet.create({
  content: { padding: 16, paddingBottom: 55 }, heading: { alignItems: "center", flexDirection: "row", justifyContent: "space-between", marginBottom: 13, marginTop: 8 },
  ...
  hall:{gap:8},hallHead:{alignItems:"center",flexDirection:"row",gap:8},hallName:{color:colors.text,fontSize:16,fontWeight:"900"},hallMeta:{color:colors.muted,fontSize:10,lineHeight:15,marginTop:3},hallBenefit:{color:colors.green,fontSize:10,fontWeight:"800",lineHeight:15},contract: { gap: 9, marginBottom: 8 },contractAttention:{borderColor:colors.gold},contractTop:{alignItems:"center",flexDirection:"row",gap:9},renewals:{flexDirection:"row",flexWrap:"wrap",gap:6}, flex: { flex: 1 }, heroName: { color: colors.text, fontSize: 16, fontWeight: "900" }, contractMeta: { color: colors.muted, fontSize: 11, marginTop: 3 }, contractRight: { alignItems: "flex-end", gap: 3 },

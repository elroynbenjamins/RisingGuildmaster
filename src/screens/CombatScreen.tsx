...
export function CombatScreen({ questId, heroes, combatSetup, initialHeroInstances, onQuestEnd, onEnemiesEncountered, onExit }: { questId: string; heroes: Hero[]; combatSetup?: QuestCombatSetup; initialHeroInstances?: HeroCombatInstance[]; onQuestEnd(status: "victory" | "defeat", instances: HeroCombatInstance[]): void; onEnemiesEncountered?(enemyDefinitionIds: string[]): void; onExit?(): void }) {
  ...
  const [state, setState] = useState(() => createCombatState(questId, 0, heroes, random.current, initialHeroInstances, combatSetup));
  ...
  const reachableKeys = useMemo(() => new Set(current && mode === "move" && !state.actions.movementUsed ? getReachablePositions(state.board, current.unit.position, current.unit.movementRange, current.unit.ignoredTerrainMovementCosts).map(positionKey) : []), [current, mode, state.actions.movementUsed, state.board]);
  ...
  const continueQuest = () => { const carried = state.heroes.map((item) => recoverBetweenEncounters(item.instance)); setState(createCombatState(questId, state.encounterIndex + 1, heroes, random.current, carried, combatSetup)); resetSelection(); };

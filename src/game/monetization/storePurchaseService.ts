...
export async function fulfillStorePurchase(
  ...
  if (removesAds) {
    ...
    const deliveredGems = pack.gems + pack.bonusGems;
    await handlers.creditGems({ transactionId: `google-play-${purchase.transactionId}`, source: "purchase", gems: deliveredGems, verified: true, note: `${pack.name} purchased through Google Play (${pack.gems} + ${pack.bonusGems} bonus gems)` });
  }

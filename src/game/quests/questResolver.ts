...
export function getQuestXpForHero(hero: Hero, quest: ReturnType<typeof getQuestDefinition>, partySize = 1): number {
  ...
  // Preserve the former mid-range progression pace without hero-specific potential.
  const progressionXp = Math.round(developmentBase * 1.25);
  if (!quest.repeatable || quest.recommendedLevelMax === undefined || hero.level <= quest.recommendedLevelMax) return progressionXp;
  const levelsAbove = hero.level - quest.recommendedLevelMax;
  return Math.max(1, Math.round(progressionXp * Math.max(.1, 1 - levelsAbove * .25)));
}
...
export function resolveQuestVictory(activeQuest: ActiveQuest, party: Party, guild: GuildState, instances: readonly HeroCombatInstance[], random: RandomSource): { activeQuest: ActiveQuest; guild: GuildState } {
  ...
  if (quest.huntReward) {
    ...
    const rolled = quest.questType === "side" && !quest.repeatable && !quest.hiddenFromQuestBoard
      ? { amount: quest.huntReward.firstVictoryCount, progress: { victories: previous.victories + 1, failuresSinceFragment: 0 } }
      : rollHuntFragment(previous, quest.huntReward.firstVictoryCount, quest.huntReward.repeatDropChance, quest.huntReward.pityAfterFailures, random);

...
describe("central guild calendar and economy", () => {
  it("restores ten percent max HP to every living hero when a day passes", () => {
    const wounded = { ...testHero(), id: "wounded", currentHP: 100 };
    const nearlyFull = { ...testHero(), id: "nearly-full", currentHP: calculateHero(testHero()).stats.maxHP - 2 };
    const fallen = { ...testHero(), id: "fallen", currentHP: 0, isAvailable: false };
    const guild = createGuild(); guild.heroes = [wounded, nearlyFull, fallen];
    const result = advanceGuildTime(guild);
    const maxHP = calculateHero(wounded).stats.maxHP;

    expect(result.guild.heroes.find((hero) => hero.id === wounded.id)?.currentHP).toBe(100 + Math.round(maxHP * GAME_CONFIG.dailyHeroHealthRecoveryRatio));
    expect(result.guild.heroes.find((hero) => hero.id === nearlyFull.id)?.currentHP).toBe(maxHP);
    expect(result.guild.heroes.find((hero) => hero.id === fallen.id)?.currentHP).toBe(0);
    expect(result.days[0]?.events).toContainEqual(expect.objectContaining({ type: "health_recovered" }));
  });
  ...
  it("previews and resolves all existing day-based systems together", () => {
    ...
    expect(new Set(result.days[0]?.events.map((event) => event.type))).toEqual(new Set(["tavern_income", "health_recovered", "workshop_complete", "gathering_ready", "scout_ready", "condition_recovered", "candidate_expired", "regional_threat"]));
  });
  ...
  it("recovers 35 readiness stamina when a day ends", () => {
    const hero = { ...testHero(), id: "resting-hero", adventureStamina: 20 };
    const guild = createGuild(); guild.heroes = [hero];
    const result = advanceGuildTime(guild);
    expect(GAME_CONFIG.adventureStaminaRecoveryPerDay).toBe(35);
    expect(result.guild.heroes[0]?.adventureStamina).toBe(55);
  });

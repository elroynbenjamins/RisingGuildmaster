...
export function CombatBoard({ board, units, labels, enemyPortraitIds = {}, heroPortraits = {}, reachableKeys = new Set(), targetableKeys = new Set(), affectedKeys = new Set(), selectedUnitId, selectedPosition, onTilePress }: { board: CombatBoardState; units: readonly CombatUnit[]; labels: Readonly<Record<string, string>>; enemyPortraitIds?: Readonly<Record<string, string>>; heroPortraits?: Readonly<Record<string, Hero>>; reachableKeys?: ReadonlySet<string>; targetableKeys?: ReadonlySet<string>; affectedKeys?: ReadonlySet<string>; selectedUnitId?: string; selectedPosition?: GridPosition; onTilePress(position: GridPosition, occupantId: string | null): void }) {
  const { guild } = useGuild();
  const byId = new Map(units.map((unit) => [unit.combatantId, unit]));
  return <View accessibilityLabel={`${board.width} by ${board.height} tactical combat board`} style={styles.board}>{board.tiles.map((tile) => { const key = positionKey(tile.position); const occupant = tile.occupantId ? byId.get(tile.occupantId) : undefined; const unit = occupant?.isAlive ? occupant : undefined; const hero = unit?.side === "heroes" ? heroPortraits[unit.combatantId] ?? guild.heroes.find((item) => item.id === unit.combatantId) : undefined; return <CombatTile key={key} tile={tile} columns={board.width} reachable={reachableKeys.has(key)} targetable={targetableKeys.has(key)} affected={affectedKeys.has(key)} selected={selectedPosition ? positionKey(selectedPosition) === key : false} onPress={() => onTilePress(tile.position, unit?.combatantId ?? null)}>{unit && <CombatToken label={labels[unit.combatantId] ?? "?"} side={unit.side} enemyDefinitionId={enemyPortraitIds[unit.combatantId]} hero={hero} size={board.width >= 13 ? 20 : 27} selected={unit.combatantId === selectedUnitId} attackable={targetableKeys.has(key)} />}</CombatTile>; })}</View>;
}

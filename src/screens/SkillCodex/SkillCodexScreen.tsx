...
export function SkillCodexScreen({ onBack }: { onBack(): void }) {
  const { guild } = useGuild(); const [tab, setTab] = useState<Tab>("Hero Skills");
  const heroSources = useMemo(() => { const sources: Record<string, string[]> = {}; for (const heroClass of Object.values(CLASSES)) for (const id of heroClass.skillIds) (sources[id] ??= []).push(heroClass.name); for (const subclass of Object.values(SUBCLASSES)) for (const id of subclass.addedSkillIds) (sources[id] ??= []).push(subclass.name); return sources; }, []);

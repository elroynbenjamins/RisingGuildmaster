...
export function manualRefreshRecruitment(guild: GuildState, random: RandomSource): GuildState { if (!getDifficulty(guild.difficultyId).allowsPaidRecruitmentRefresh && !canUsePaidRecruitmentRefresh(guild)) throw new Error("Iron Guild gold refreshes return after the Brambleford company trial"); if (guild.gold < guild.recruitment.manualRefreshCost) throw new Error("Insufficient Gold"); const reserved = guild.recruitment.candidates.filter((candidate) => candidate.candidateId === guild.recruitment.reservedCandidateId && (guild.recruitment.reservationExpiresAtDay ?? 0) > guild.currentDay); const generated = unlockedPool(guild, random); return { ...guild, gold: guild.gold - guild.recruitment.manualRefreshCost, recruitment: { ...withCandidates(guild.recruitment, [...reserved, ...generated]), batchRecruitmentUsed: false } }; }
export function freeRefreshRecruitment(guild: GuildState, random: RandomSource): GuildState { if (guild.currentDay < guild.recruitment.nextFreeRefreshDay) throw new Error("Free refresh is not ready"); const reserved = guild.recruitment.candidates.filter((candidate) => candidate.candidateId === guild.recruitment.reservedCandidateId && (guild.recruitment.reservationExpiresAtDay ?? 0) > guild.currentDay); const generated = unlockedPool(guild, random); return { ...guild, recruitment: { ...withCandidates(guild.recruitment, [...reserved, ...generated]), lastFreeRefreshDay: guild.currentDay, nextFreeRefreshDay: guild.currentDay + RECRUITMENT_CONFIG.freeRefreshDays, batchRecruitmentUsed: false } }; }
export function tutorialRefreshRecruitment(guild: GuildState, random: RandomSource): GuildState { const reserved = guild.recruitment.candidates.filter((candidate) => candidate.candidateId === guild.recruitment.reservedCandidateId); const generated = unlockedPool(guild, random); return { ...guild, recruitment: { ...withCandidates(guild.recruitment, [...reserved, ...generated]), batchRecruitmentUsed: false } }; }

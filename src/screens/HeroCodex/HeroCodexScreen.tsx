...
import { BASE_CLASS_IDS, BASE_RACE_IDS, STORY_RACE_UNLOCKS } from "../../game/monetization/contentUnlockService";
...
function RaceEntry({ raceId, unlocked }: { raceId: RaceId; unlocked: boolean }) {
...
export function HeroCodexScreen({ onBack }: { onBack(): void }) {
  ...
  const normalized=query.trim().toLowerCase();const raceIds=(Object.keys(RACES) as RaceId[]).filter((id)=>!normalized||RACES[id].name.toLowerCase().includes(normalized)||RACE_CODEX[id].epithet.toLowerCase().includes(normalized));const classIds=(Object.keys(CLASSES) as ClassId[]).filter((id)=>!normalized||CLASSES[id].name.toLowerCase().includes(normalized)||CLASS_CODEX[id].role.toLowerCase().includes(normalized));
  return <ScrollView keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag" contentContainerStyle={styles.content}><BackButton onPress={onBack} /><Text style={styles.eyebrow}>GUILD ARCHIVES</Text><Text style={styles.title}>Heroes' Codex</Text><Text style={styles.intro}>A living field guide to {Object.keys(RACES).length} peoples and {Object.keys(CLASSES).length} guild disciplines. Locked entries explain how they permanently join every save's recruitment pool.</Text><ArchiveRewardsPanel track="heroes" /><TextInput accessibilityLabel="Search Heroes' Codex" placeholder="Search by name or role…" placeholderTextColor={colors.muted} value={query} onChangeText={setQuery} autoCorrect={false} autoCapitalize="none" returnKeyType="done" submitBehavior="blurAndSubmit" style={{backgroundColor:colors.panel,borderColor:colors.border,borderRadius:9,borderWidth:1,color:colors.text,marginBottom:12,paddingHorizontal:12,paddingVertical:10}}/><SegmentedTabs values={["Races", "Classes"] as const} value={tab} onChange={setTab} /><SectionTitle>{tab === "Races" ? `PEOPLES OF ELDORIA · ${raceIds.length}` : `ADVENTURING CLASSES · ${classIds.length}`}</SectionTitle>{tab === "Races" ? raceIds.map((id) => <RaceEntry key={id} raceId={id} unlocked={guild.entitlements.unlockedRaceIds.includes(id)} />) : classIds.map((id) => <ClassEntry key={id} classId={id} unlocked={guild.entitlements.unlockedClassIds.includes(id)} />)}</ScrollView>;
}

const styles = StyleSheet.create({ content: { padding: 20, paddingBottom: 40 }, eyebrow: { color: colors.gold, fontSize: 11, fontWeight: "900", letterSpacing: 2, marginTop: 16 }, title: { color: colors.text, fontSize: 30, fontWeight: "900", marginTop: 3 }, intro: { color: colors.muted, lineHeight: 20, marginBottom: 16, marginTop: 8 }, entry: { marginBottom: 10 }, locked: { opacity: .72, borderStyle: "dashed" }, header: { alignItems: "center", flexDirection: "row" }, headerText: { flex: 1, marginLeft: 12 }, entryTitle: { color: colors.text, fontSize: 19, fontWeight: "900" }, meta: { color: colors.muted, fontSize: 12, marginTop: 2 }, chevron: { color: colors.gold, fontSize: 24, fontWeight: "700" }, details: { borderTopColor: colors.border, borderTopWidth: 1, marginTop: 14, paddingTop: 13 }, source: { color: colors.gold, fontSize: 11, fontWeight: "900", marginBottom: 9, textTransform: "capitalize" }, body: { color: colors.text, fontSize: 14, lineHeight: 20 }, label: { color: colors.gold, fontSize: 11, fontWeight: "900", letterSpacing: 1.2, marginBottom: 7, marginTop: 15 }, chips: { flexDirection: "row", flexWrap: "wrap", gap: 7 }, chip: { backgroundColor: colors.panel2, borderColor: colors.border, borderRadius: 7, borderWidth: 1, color: colors.text, fontSize: 12, overflow: "hidden", paddingHorizontal: 9, paddingVertical: 6 }, growthRow: { flexDirection: "row", gap: 5 }, growthCell: { alignItems: "center", backgroundColor: colors.panel2, borderRadius: 7, flex: 1, paddingVertical: 7 }, growthLabel: { color: colors.muted, fontSize: 9, fontWeight: "900" }, growthValue: { color: colors.text, fontSize: 12, fontWeight: "800", marginTop: 2 }, skill: { backgroundColor: colors.panel2, borderRadius: 9, marginBottom: 8, padding: 11 }, skillHeading: { alignItems: "center", flexDirection: "row", justifyContent: "space-between" }, skillName: { color: colors.text, flex: 1, fontSize: 14, fontWeight: "800" }, skillType: { color: colors.gold, fontSize: 9, fontWeight: "900" }, skillLine: { color: colors.muted, fontSize: 12, lineHeight: 17, marginTop: 4 } });

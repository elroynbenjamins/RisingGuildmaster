...
export function validateParty(party: Party, heroes: readonly Hero[]): PartyValidationResult {
  const errors: string[] = [];
  if (party.heroIds.length < MIN_PARTY_SIZE) errors.push("Party must contain at least one hero");
  if (party.heroIds.length > MAX_PARTY_SIZE) errors.push("Party cannot contain more than four heroes");

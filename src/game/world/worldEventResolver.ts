...
export function resolveAbilityCheck(check: AbilityCheck, heroes: readonly Hero[], random: RandomSource, optionalModifier = 0): AbilityCheckResult {
  ...
  const attribute = calculateHero(hero).attributes[check.attribute]; const modifier = Math.floor(attribute / 4) + optionalModifier; const diceRoll = rollDie(20, random); const total = diceRoll + modifier;

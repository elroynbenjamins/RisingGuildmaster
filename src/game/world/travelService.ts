...
export function buyRations(guild: GuildState): GuildState { if (!guild.world.currentSettlementId) throw new Error("Rations can only be bought in a settlement"); if (!isSettlementAvailable(guild.world, guild.world.currentSettlementId)) throw new Error("This settlement is unavailable because of a regional crisis"); if (guild.gold < GAME_CONFIG.rationBundleGoldCost) throw new Error("Not enough gold"); return { ...guild, gold: guild.gold - GAME_CONFIG.rationBundleGoldCost, rations: guild.rations + getRationBundleAmount(guild) }; }

...
import { advanceCombatTutorial } from "../src/game/onboarding/combatTutorialService";

describe("new-game onboarding and progression gates", () => {
  it("advances guided combat only after the expected successful action", () => {
    expect(advanceCombatTutorial("move", "ended_turn")).toBe("move");
    expect(advanceCombatTutorial("move", "moved")).toBe("basic_attack");
    expect(advanceCombatTutorial("basic_attack", "moved")).toBe("basic_attack");
    expect(advanceCombatTutorial("basic_attack", "used_basic_attack")).toBe("end_turn");
  ...
  it("guides two recruits without comparison through one mandatory free refresh", () => {
    ...
    expect(guild.tutorial.step).toBe("inspect_candidate");
    guild = recordTutorialCandidateTab(guild, "candidate-1", "Overview");
    guild = recordTutorialCandidateTab(guild, "candidate-1", "Contract");
    expect(guild.tutorial.step).toBe("inspect_candidate");
    guild = recordTutorialCandidateTab(guild, "candidate-1", "Stats");
    expect(guild.tutorial.step).toBe("recruit_first");
    expect(guild.tutorial.inspectedCandidateTabs).toEqual(["Overview", "Contract", "Stats"]);
  ...
  it("accepts Traits as the alternative core candidate review", () => {
    let guild = beginTutorial(createGuild());
    guild = recordTutorialCandidateTab(guild, "candidate-1", "Overview");
    guild = recordTutorialCandidateTab(guild, "candidate-1", "Traits");
    expect(guild.tutorial).toMatchObject({ step: "recruit_first", inspectedCandidateTabs: ["Overview", "Traits"] });
  });

...
export function RecruitmentScreen({ onBack, inspect, openCalendar, openCampaign }: { onBack(): void; inspect(candidate: RecruitmentCandidate): void; openCalendar(): void; openCampaign(): void }) {
  ...
  const [sort, setSort] = useState<"Fee" | "Salary" | "Expiry">("Fee");
  const [showCandidateTools, setShowCandidateTools] = useState(false);
  const displayedCandidates = candidates.filter(candidate => sourceFilter === "All" || candidate.source === (sourceFilter === "Scout" ? "regional_scout" : "guild_board")).filter(candidate => `${candidate.heroPreview.name} ${RACES[candidate.heroPreview.raceId].name} ${CLASSES[candidate.heroPreview.classId].name}`.toLowerCase().includes(query.trim().toLowerCase())).sort((a,b) => sort === "Fee" ? a.recruitmentFeeEstimateMin - b.recruitmentFeeEstimateMin : sort === "Salary" ? a.weeklySalaryEstimateMin - b.weeklySalaryEstimateMin : sort === "Expiry" ? a.expiresAtDay - b.expiresAtDay : a.recruitmentFeeEstimateMin - b.recruitmentFeeEstimateMin);
  ...
  useEffect(() => {
    if (!tutorial) { refreshPulse.setValue(1); return; }
    ...
    pulse.start();
    return () => pulse.stop();
  }, [tutorial, tutorialRefresh, refreshPulse]);
  ...
  return <ScrollView ref={scrollRef} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag" contentContainerStyle={styles.content}>
    ...
    <Panel style={styles.guildPlate}>
      ...
      <Pressable accessibilityRole="button" accessibilityLabel="Party role coverage" aria-expanded={showPartyFit} accessibilityState={{ expanded: showPartyFit }} onPress={() => setShowPartyFit(value => !value)} style={({pressed})=>[styles.roleCoverage,pressed&&styles.pressed]}><Text style={{ color: colors.muted, fontSize: 11 }}>Party role coverage {showPartyFit ? "−" : "+"}</Text></Pressable>
    ...
    {!tutorial && <View style={styles.candidateTools}>
      ...
      {showCandidateTools && <View style={styles.toolPanel}><SegmentedTabs values={["All", "Board", "Scout"] as const} value={sourceFilter} onChange={setSourceFilter} /><SegmentedTabs values={["Fee", "Salary", "Expiry"] as const} value={sort} onChange={setSort} /></View>}
    ...
    {displayedCandidates.map((candidate) => {
      ...
      return <Panel key={candidate.candidateId} style={[styles.card, { borderColor: cardBorder }]}>
        ...
        <Text style={styles.fitDetail}>{presentation.fitLabel} · {candidate.contractLengthWeeks} weeks</Text><Pressable accessibilityRole="button" accessibilityState={{expanded:expandedCandidateId===candidate.candidateId}} style={({pressed})=>[styles.disclosure,pressed&&styles.pressed]} onPress={()=>setExpandedCandidateId(expandedCandidateId===candidate.candidateId?undefined:candidate.candidateId)}><Text style={styles.expand}>{expandedCandidateId===candidate.candidateId?"Hide details":"More details +"}</Text></Pressable>{expandedCandidateId===candidate.candidateId&&<><Text style={styles.fitDetail}>{presentation.fitDetail}</Text>{getRecruitmentRecommendation(guild, candidate) && <Text style={styles.fitDetail}>Recommended by {getRecruitmentRecommendation(guild, candidate)!.heroName}: {getRecruitmentRecommendation(guild, candidate)!.reason}.</Text>}<View style={styles.traitRow}><Text style={styles.traits}>TRAITS · </Text>{hero.traitIds.map((id) => <Text key={id} style={[styles.traitName, { color: TRAITS[id].polarity === "positive" ? colors.green : TRAITS[id].polarity === "negative" ? colors.danger : colors.gold }]}>{TRAITS[id].name.toUpperCase()}</Text>)}</View><View style={styles.economyLine}><StatusChip label={presentation.economyLabel} tone={presentation.economyTone} /><Text style={styles.economyText}>After min. fee: {presentation.goldAfterMinimumFee} G · Payroll after hire: {range(presentation.payrollAfterMin, presentation.payrollAfterMax)} G/week</Text></View></>}

        <View style={styles.actions}><Animated.View style={[styles.flex, tutorial === "inspect_candidate" && { opacity: refreshPulse }]}><SecondaryButton label="Inspect" onPress={() => inspect(candidate)} /></Animated.View><Animated.View style={[styles.flex, (tutorial === "recruit_first" || tutorial === "recruit_second") && { opacity: refreshPulse }]}><ActionButton iconId={!hireBlock?"recruitment":undefined} label={hireBlock ?? "Hire"} disabled={Boolean(hireBlock)} onPress={() => confirmRecruit(candidate)} /></Animated.View></View>
        <View style={styles.secondaryActions}><Pressable accessibilityRole="checkbox" accessibilityState={{checked:compareIds.includes(candidate.candidateId)}} onPress={() => toggleCompare(candidate.candidateId)} style={({pressed})=>[styles.compare,compareIds.includes(candidate.candidateId)&&styles.compareSelected,pressed&&styles.pressed]}><Text style={styles.compareText}>{compareIds.includes(candidate.candidateId) ? "✓ IN COMPARISON" : "COMPARE"}</Text></Pressable><Pressable accessibilityRole="button" accessibilityState={{disabled:Boolean(tutorial)}} style={({pressed})=>[styles.rejectButton,pressed&&!tutorial&&styles.pressed]} disabled={Boolean(tutorial)} onPress={() => act(() => rejectCandidate(candidate.candidateId), `${hero.name} left the candidate board.`)}><Text style={[styles.reject, tutorial && styles.disabled]}>PASS</Text></Pressable></View>
      </Panel>;
    })}

    {compared.length >= 2 && <Panel style={styles.comparison}><Text style={styles.panelEyebrow}>GUILDMASTER'S DESK</Text><Text style={styles.name}>Candidate Comparison</Text><View style={styles.compareGrid}>{compared.map((candidate) => { const classDef = CLASSES[candidate.heroPreview.classId]; const matchingAttributes = classDef.attributePriorities.slice(0, 3); return <View key={candidate.candidateId} style={styles.compareColumn}><Text numberOfLines={1} style={[styles.columnName, { color: getRaceNameColor(candidate.heroPreview.raceId) }]}>{candidate.heroPreview.name}</Text><Text style={styles.meta}>{classDef.name} · Lv {candidate.heroPreview.level}</Text><Text style={styles.meta}>Fee {range(candidate.recruitmentFeeEstimateMin, candidate.recruitmentFeeEstimateMax)} G</Text><Text style={styles.meta}>Pay {range(candidate.weeklySalaryEstimateMin, candidate.weeklySalaryEstimateMax)} G/wk</Text>{matchingAttributes.map((key) => { const estimate = candidate.attributeEstimates[key]; return <Text key={key} style={styles.matchingAttribute}>{key.toUpperCase()} {range(estimate.minimum, estimate.maximum)}</Text>; })}</View>; })}</View></Panel>}
    {tutorial && tutorial !== "party_complete" ? <Pressable accessibilityRole="button" onPress={() => updateGuild(skipTutorial(guild))} style={styles.skipTutorial}><Text style={styles.skipTutorialText}>Skip tutorial</Text></Pressable> : null}
  </>}{section==="Alumni"&&(formerMembers.length>0?<><Text style={styles.formerHeading}>GUILD ALUMNI · {formerMembers.length}</Text><Text style={styles.formerIntro}>Departed heroes keep their history here. Contract alumni may return after time away; retired veterans remain honored members of the guild's story.</Text>{formerMembers.map((member)=>{const retired=member.departureKind==="retired";const eligible=!retired&&member.eligibleReturnDay<=guild.currentDay;const fee=retired?0:formerMemberRehireFee(member);const salary=retired?0:formerMemberRehireSalary(member);return <Panel key={member.hero.id} style={[styles.formerCard,!eligible&&!retired&&styles.formerCooling,retired&&styles.retiredCard]}><View style={styles.row}><Portrait hero={member.hero} size={62}/><View style={styles.flex}><Text style={[styles.name,{color:getRaceNameColor(member.hero.raceId)}]}>{member.hero.name}</Text><Text style={styles.meta}>{RACES[member.hero.raceId].name} {CLASSES[member.hero.classId].name} · Lv {member.hero.level}</Text><Text style={[styles.formerMeta,retired&&styles.retiredMeta]}>{retired?`Retired Day ${member.departedDay} · HONORED ALUMNUS`:`Left Day ${member.departedDay} · ${eligible?"AVAILABLE TO RETURN":"Can return Day "+member.eligibleReturnDay}`}</Text><Text style={styles.cost}>{retired?`${member.hero.history.questsCompleted} completed quests · full chronicle preserved`:`Return fee ${fee} · Salary ${salary}/wk · 12-week contract`}</Text></View></View><ActionButton iconId={!retired&&eligible&&guild.heroes.length<RECRUITMENT_CONFIG.heroCapacity&&guild.gold>=fee?"recruitment":undefined} label={retired?"Retired Veteran":!eligible?"Away for "+(member.eligibleReturnDay-guild.currentDay)+" more day(s)":guild.heroes.length>=RECRUITMENT_CONFIG.heroCapacity?"Roster Full":guild.gold<fee?"Need "+fee+" Gold":"Invite Back"} disabled={retired||!eligible||guild.heroes.length>=RECRUITMENT_CONFIG.heroCapacity||guild.gold<fee} onPress={()=>confirmRehire(member.hero.id)}/></Panel>;})}</>:<Panel style={styles.formerCard}><Text style={styles.formerHeading}>NO GUILD ALUMNI YET</Text><Text style={styles.formerIntro}>Heroes who leave after a completed contract will be recorded here and may become available to recruit again later.</Text></Panel>)}</ScrollView>;
}
...
const styles = StyleSheet.create({ cost: {color: colors.text, fontWeight: "800", marginTop: 5},formerHeading:{color:colors.gold,fontSize:10,fontWeight:"900",letterSpacing:1,marginBottom:4,marginTop:4},formerIntro:{color:colors.muted,fontSize:10,lineHeight:15,marginBottom:8},formerCard:{borderColor:"#705b3e",gap:8,marginBottom:8},formerCooling:{opacity:.64},retiredCard:{borderColor:"#7e7459"},retiredMeta:{color:colors.gold},formerMeta:{color:colors.green,fontSize:9,fontWeight:"900",marginTop:4},
  ...
  eyebrow: {color: colors.gold, fontSize: 10, fontWeight: "900", letterSpacing: .8, marginTop: 0},
  ...
  plateLabel: { color: colors.muted, fontSize: 8, fontWeight: "900", letterSpacing: .7 },
  ...
  statLabel: { color: colors.muted, fontSize: 7, fontWeight: "900", letterSpacing: .6 },
  ...
  toolPanel: { gap: 7 },
  panelEyebrow: { color: colors.muted, fontSize: 8, fontWeight: "900", letterSpacing: .7 },
  ...
  candidateName: {flexShrink: 1, fontWeight: "900", fontSize: 20, lineHeight: 25},
  ...
  offerLabel: {color: colors.muted, fontWeight: "900", letterSpacing: .5, fontSize: 9},
  offerValue: { color: colors.text, fontSize: 13, fontWeight: "900", marginTop: 3 },
  fitDetail: { color: colors.muted, fontSize: 10, lineHeight: 15, marginTop: 7 }, disclosure:{justifyContent:"center",minHeight:44}, expand:{color:colors.gold,fontSize:10,fontWeight:"800"},
  traits: { color: colors.muted, fontSize: 10, fontWeight: "800", lineHeight: 15 }, traitRow:{flexDirection:"row",flexWrap:"wrap",gap:6,marginTop:6}, traitName:{fontSize:10,fontWeight:"900",lineHeight:15},
  ...
  roleCoverage:{minHeight:44,justifyContent:"center"}, compare: {borderColor: colors.border, paddingHorizontal: 10, paddingVertical: 7, borderWidth: 1, borderRadius: 10, minHeight: 44, justifyContent: "center"}, compareSelected:{borderColor:colors.gold,backgroundColor:colors.panel2},
  compareText: {letterSpacing: .5, fontWeight: "800", fontSize: 11, color: colors.muted},
  rejectButton:{alignItems:"center",justifyContent:"center",minHeight:44}, reject: {color: colors.danger, paddingHorizontal: 12, fontWeight: "800", fontSize: 11},
  disabled: { opacity: .35 }, pressed:{opacity:.76,transform:[{translateY:1}]},
  ...
  matchingAttribute: { color: colors.green, fontSize: 10, fontWeight: "900", marginTop: 3 },
  skipTutorial:{alignItems:"center",justifyContent:"center",minHeight:44,marginTop:4},skipTutorialText:{color:colors.muted,fontSize:11,fontWeight:"700"},
});

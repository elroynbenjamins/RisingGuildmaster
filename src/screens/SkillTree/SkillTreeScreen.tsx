...
export function SkillTreeScreen({ hero, onBack, onUpdate, openClassPath }: { hero: Hero; onBack(): void; onUpdate(hero: Hero): void; openClassPath?(): void }) {
  ...
  const learn = (skillId: string) => {
    ...
    try { const next=learnClassSkill(hero,skillId); setError(undefined); onUpdate(next); setConfirmLearning(false); triggerTactileFeedback(guild.uiPreferences.tactileFeedback,"confirm"); showToast({title:"Skill Learned",message:`${hero.name} learned ${HERO_SKILLS[skillId]?.name ?? "a new skill"}.`,tone:"success"}); }
    catch (caught) { const message=caught instanceof Error ? caught.message : "Could not learn skill"; setError(message); triggerTactileFeedback(guild.uiPreferences.tactileFeedback,"warning"); showToast({title:"Skill Choice Failed",message,tone:"danger"}); }
  };

...
describe("world travel supplies", () => {
  ...
  it("charges a day for walking to another settlement and localizes side quests", () => {
    ...
    expect(isQuestAvailableAtCurrentLocation(QUESTS.brambleway_caravan!, moved.world)).toBe(false);
    const roadDefended = { ...moved.world, worldFlags: { ...moved.world.worldFlags, starter_brambleway_road_ambush_complete: true } };
    expect(isQuestAvailableAtCurrentLocation(QUESTS.brambleway_caravan!, roadDefended)).toBe(true);

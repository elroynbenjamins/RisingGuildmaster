...
export function HeroRelationshipsPanel({ heroId }: { heroId: string }) {
  ...
  return <><SectionTitle>RELATIONSHIPS</SectionTitle><Panel>{entries.length ? entries.map((item) => { const band = relationshipBand(item.score); return <View key={`${item.heroIdA}-${item.heroIdB}`} style={styles.entry}><View style={styles.row}><Text style={[styles.name,{color:themeColors.text}]}>{item.partner!.name}</Text><Text style={[styles.score, { color: item.score < -20 ? themeColors.danger : item.score > 20 ? themeColors.green : themeColors.gold }]}>{RELATIONSHIP_BAND_LABELS[band]} · {item.score}</Text></View><Text style={[styles.effect,{color:themeColors.muted}]}>{band === "friend" || band === "close_friend" ? "Adjacent in combat: +1 attack roll." : band === "rival" ? "Together in combat: +5% physical damage; healing each other is 10% weaker." : "No combat consequence at this relationship level."}</Text></View>; }) : <Text style={[styles.empty,{color:themeColors.muted}]}>Complete quests together to form friendships, rivalries, and shared history.</Text>}</Panel></>;
}

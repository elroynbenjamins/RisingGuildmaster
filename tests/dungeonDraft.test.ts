...
function guildWithHeroes(count: number) { const guild = createGuild(); return { ...guild, heroes: Array.from({ length: count }, (_, index) => ({ ...generateHero(createSeededRandom(index + 50)), id: `draft-${index}`, isAvailable: true, currentHP: 100 })) }; }

...
export function getSkillDescriptionLines(skill: CombatSkillDefinition): string[] {
  ...
  if (skill.companion) lines.push(`Binds one ${words(skill.companion.id)} for ${skill.companion.durationTurns} turns with ${Math.round(skill.companion.hpMultiplier * 100)}% hero HP and ${Math.round(skill.companion.damageMultiplier * 100)}% hero ${words(skill.companion.damageScalingStat)} as ${skill.companion.damageType} pressure. It acts on the summoner's initiative.`);

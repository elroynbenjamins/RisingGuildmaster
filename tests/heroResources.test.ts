...
describe("hero combat resources", () => {
  ...
  it("recovers resources but not HP by default between encounters", () => expect(recoverBetweenEncounters(instance({ currentHP: 25 }))).toMatchObject({ currentHP: 25, currentMana: 60, currentStamina: 60 }));
  it("supports authored between-wave HP recovery", () => expect(recoverBetweenEncounters(instance({ currentHP: 25 }), .10)).toMatchObject({ currentHP: 35, currentMana: 60, currentStamina: 60 }));
});

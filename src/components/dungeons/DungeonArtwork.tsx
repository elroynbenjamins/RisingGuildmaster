...
export function DungeonArtwork({ variant, compact = false }: { variant: DungeonArtworkVariant; compact?: boolean }) {
  ...
  return <View accessibilityRole="image" accessibilityLabel={`${variant} dungeon artwork`} style={[styles.viewport, { width: panelWidth, height: panelHeight }]}>
...
export function DungeonNodeArtworkIcon({ type, size = 34 }: { type: DungeonNodeType | "locked"; size?: number }) {
  ...
  return <View style={[styles.iconViewport, { width: size, height }]}>

...
Heroes and guild state contain JSON-safe data only. Definitions and functions are resolved from IDs when calculating. Persistent saves use a versioned envelope rather than treating raw `GuildState` JSON as an unversioned storage contract.

- `src/game/save/saveSchema.ts` owns serialized envelope versions and explicit `vN -> vN+1` migrations.
- `src/game/save/guildStateMigration.ts` owns historical field/default normalization into the current runtime `GuildState`.
- `src/game/save/saveService.ts` owns AsyncStorage I/O, two-slot backup rotation, recovery, and the legacy single-save bridge.

Each slot keeps one validated backup. Old raw saves are upgraded only after they have loaded successfully; saves from a newer unsupported schema are rejected instead of guessed at. See `docs/save-schema.md`.

...
export function QuestDecisionScreen({ party, onBack, onComplete }: { party: Party; onBack(): void; onComplete(setup: QuestCombatSetup): void }) {
  ...
  const canBack=stageIndex===0&&!result&&progress.results.length===0;
  return <ScrollView contentContainerStyle={styles.content}>{canBack?<BackButton onPress={onBack} />:null}<Text style={styles.eyebrow}>SIDE QUEST · DECISION {stageIndex + 1}/{stages.length}</Text><Text style={styles.title}>{stage.title}</Text><Text style={styles.story}>{stage.description}</Text>

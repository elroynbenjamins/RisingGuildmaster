...
function Game() {
  ...
  if (isHydrated && route.name === "dungeonCombat") {
    ...
    return <CombatScreen questId="wardstone_depths_expedition" heroes={participants} initialHeroInstances={run.heroInstances} combatSetup={getDungeonCombatSetup(guild)} initialCombatState={run.combatState ?? undefined} initialRandomState={run.combatRandomState ?? undefined} onCombatCheckpoint={(state,randomState)=>updateGuild((current)=>current.activeDungeonRun?.id===run.id?checkpointDungeonCombat(current,state,randomState):current)} onEnemiesEncountered={(ids) => updateGuild((current)=>discoverEnemies(current, ids))} onQuestEnd={finish}/>;
  }

...
describe("quest loop", () => {
  ...
  it("combines shared enemy and objective XP at the standard hero progression rate", () => { const quest = QUESTS.goblin_patrol!; expect(getQuestEnemyXpPool(quest)).toBe(300); expect(getQuestXpForHero(testHero(), quest, 2)).toBe(563); });
  ...
  it("awards shared XP, gold, loot, materials and history", () => { const healthy = testHero(); const fallen = { ...testHero(), id: "h2" }; const guild = { ...createGuild(), heroes: [healthy, fallen] }; const party = { id: "p", heroIds: [healthy.id, fallen.id] }; const result = resolveQuestVictory(startQuest(QUESTS.goblin_patrol!, party), party, guild, [instance(), instance({ heroId: "h2", currentHP: 0, isAlive: false })], sequenceRandom([0, .9, 0, .5, .5])); expect(result.guild.heroes[0]?.xp).toBe(563); expect(result.guild.heroes[1]?.xp).toBe(282); expect(result.guild.inventory).toHaveLength(1); expect(result.guild.materials.iron_ore).toBeGreaterThan(0); expect(result.guild.heroes[0]?.history.questsCompleted).toBe(1); });

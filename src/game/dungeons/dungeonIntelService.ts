...
export function getDungeonRouteLayers(dungeonId: string): string[][] {
  ...
  while (queue.length) {
    ...
    for (const nextId of DUNGEON_NODES[id]?.nextNodeIds ?? []) if (!depths.has(nextId) || depths.get(nextId)! > depth + 1) { depths.set(nextId, depth + 1); queue.push(nextId); }
  }

...
export interface TrainingProgramDefinition { id: TrainingProgramId; name: string; description: string; durationDays: number; baseGoldCost: number; baseXp: number; focusedAttribute?: AttributeKey; classWeightedGrowth: number; trainingGroundLevel: number }
export interface TrainingSession { id: string; heroId: string; programId: TrainingProgramId; startDay: number; completionDay: number; goldCost: number; xpReward: number; focusedAttribute?: AttributeKey; growthReward: number; levelCap?: number; developmentLevel?: number }

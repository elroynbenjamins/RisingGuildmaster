...
export interface QuestDefinition { id: string; name: string; description?: string; questType: QuestType; regionId: string; repeatable: boolean; hiddenFromQuestBoard?: boolean; difficulty: number; recommendedLevelMin?: number; recommendedLevelMax?: number; minPartySize: number; maxPartySize: number; explorationStageIds?: string[]; decisionStageIds?: string[]; encounterIds: string[]; goldRewardMin: number; goldRewardMax: number; xpRewardPerHero: number; lootTableId: string; huntReward?: HuntRewardDefinition; storyArcId?: string; campaignChapter?: number; prerequisiteQuestIds?: string[]; prerequisiteCampaignNodeIds?: string[]; requiredWorldFlags?: string[]; setWorldFlagsOnVictory?: Record<string, boolean> }

...
export function resolveEquipmentDefinition(key: string): ResolvedEquipmentDefinition | undefined {
  ...
  return { ...base, inventoryKey: key, enchantmentIds: enchantments.map((item) => item.id), name: enchantments.length ? `${enchantments.map((item) => item.name).join(" / ")} ${base.name}` : base.name, value: Math.round(base.value + enchantments.reduce((sum, item) => sum + item.goldCost * .5, 0)), modifiers: [...base.modifiers, ...enchantments.flatMap((item) => item.modifiers)], specialEffectIds: [...base.specialEffectIds, ...enchantments.map((item) => `enchantment:${item.id}`), ...enchantments.flatMap((item) => item.specialEffectIds ?? [])] };
}

...
describe("subclass progression", () => {
  it("gives every available class exactly two subclasses", () => { expect(Object.keys(CLASSES)).toEqual(["warrior", "ranger", "mage", "cleric", "paladin", "berserker", "monk", "bard", "spellbow", "bulwark", "summoner"]); expect(Object.keys(SUBCLASSES)).toHaveLength(22); for (const classId of Object.keys(CLASSES)) expect(Object.values(SUBCLASSES).filter((subclass) => subclass.baseClassId === classId)).toHaveLength(2); });
  it("rejects Guardian at level 4, allows both Warrior choices at 5, and makes selection permanent", () => { const level4 = { ...testHero(), level: 4 }; expect(() => selectSubclass(level4, "guardian")).toThrow(); const level5 = { ...level4, level: 5 }; expect(getSubclassChoices(level5).map((choice) => choice.id)).toEqual(["guardian", "battlemage_commander"]); const guardian = selectSubclass(level5, "guardian"); expect(guardian.subclassId).toBe("guardian"); expect(() => selectSubclass(guardian, "battlemage_commander")).toThrow(); });
  ...
  it("stores all subclass numerical special effects in shared skill definitions", () => { expect(HERO_SKILLS.beastmaster_summon_wolf?.companion).toMatchObject({ hpMultiplier: .4, damageScalingStat: "physicalDamage", damageType: "physical", durationTurns: 6, maxActive: 1 }); expect(HERO_SKILLS.pyromancer_flame_wall?.flameWall).toMatchObject({ connectedTileCount: 3, durationRounds: 2 }); expect(HERO_SKILLS.cryomancer_ice_prison?.savingThrowCondition).toMatchObject({ difficultyClass: 14, conditionId: "rooted" }); expect(HERO_SKILLS.bloodreaver_blood_strike?.lifeStealModifier).toBe(.2); });
});

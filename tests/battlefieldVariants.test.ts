...
describe("Shattered Wardstone Warfront", () => {
  it("provides a large mixed-terrain battlefield with navigable flanking lanes", () => {
    const field = BATTLEFIELDS.shattered_wardstone_warfront!;
    const board = createCombatBoard([], field.boardSizeId, field.terrainPlacements, field.id);
    expect(field).toMatchObject({ boardSizeId: "warfront" });
    expect(board).toMatchObject({ width: 15, height: 11 });
    expect(new Set(field.terrainPlacements.map((placement) => placement.terrainType))).toEqual(new Set(["shallow_water", "mountain", "forest", "barricade", "obstacle"]));
    expect(findShortestPath(board, { x: 0, y: 5 }, { x: 14, y: 5 }, 30)).not.toBeNull();
    expect(QUEST_ENCOUNTERS.goblin_chieftain_encounter?.battlefieldId).toBe(field.id);
  });
});

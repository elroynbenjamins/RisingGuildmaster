...
export function RecruitmentScreen({ onBack, inspect, openCalendar }: { onBack(): void; inspect(candidate: RecruitmentCandidate): void; openCalendar(): void }) {
  ...
  const confirmRecruit = (candidate: RecruitmentCandidate) => {
    ...
    showDialog({ title: `Recruit ${candidate.heroPreview.name}?`, message: `Estimated fee: ${range(candidate.recruitmentFeeEstimateMin, candidate.recruitmentFeeEstimateMax)} gold\nSalary demand: ${range(candidate.weeklySalaryEstimateMin, candidate.weeklySalaryEstimateMax)} gold/week\nContract: ${candidate.contractLengthWeeks} weeks${recommendation ? `\n\nRecommended by ${recommendation.heroName}: ${recommendation.reason}.` : ""}`, eyebrow: "GUILD CONTRACT", actions: [{ label: "Cancel", tone: "secondary" }, { label: "Recruit", tone: "primary", onPress: () => act(() => recruitCandidate(candidate.candidateId), `${candidate.heroPreview.name} joined the guild.`) }] });
  };

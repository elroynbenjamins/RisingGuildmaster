...
export function resolveGatheringMission(guild: GuildState, missionId: string): GatheringMissionResult {
  ...
  const definition = GATHERING_MISSIONS[mission.definitionId]!; const heroes = missionHeroes(guild, mission); const random = createSeededRandom(mission.resolutionSeed); const ghorakGuideBonus = guild.world.worldFlags.ghorak_guide_unused === true && definition.regionId === "iron_hills" ? 2 : 0; const modifier = calculateGatheringModifier(definition.id, heroes) + ghorakGuideBonus; const diceRoll = random.int(1, 20); const total = diceRoll + modifier; const success = total >= definition.difficultyClass; const margin = total - definition.difficultyClass; const averageLevel = (heroes[0].level + heroes[1].level) / 2;
...
export function claimGatheringMission(guild: GuildState, missionId: string): { guild: GuildState; result: GatheringMissionResult } {
  ...
  const usedGhorakGuide = guild.world.worldFlags.ghorak_guide_unused === true && GATHERING_MISSIONS[mission.definitionId]?.regionId === "iron_hills";
  return { result, guild: { ...guild, world: usedGhorakGuide ? { ...guild.world, worldFlags: { ...guild.world.worldFlags, ghorak_guide_unused: false } } : guild.world, materials, inventory: [...guild.inventory, ...result.equipmentIds], heroes: guild.heroes.map((hero) => { if (!mission.heroIds.includes(hero.id)) return hero; const cap = getTrainingProgressionLimit(guild, hero).levelCap; return { ...grantTrainingXp(hero, result.xpPerHero + getIdleMissionLevelProgressXp(hero.level), cap), isAvailable: hero.currentHP > 0 }; }), gatheringMissions: guild.gatheringMissions.map((item) => item.id === missionId ? { ...item, status: "claimed" } : item) } };
}

...
export function ManagementShell({ guild, active, onSelect, onOpenGems, children }: React.PropsWithChildren<{ guild: GuildState; active: MainTab; onSelect(tab: MainTab): void; onOpenGems(): void }>) { const{colors}=useTheme();return <View style={[styles.root,{backgroundColor:colors.background}]}><GlobalResourceHeader onOpenGems={onOpenGems} guild={guild} /><View style={styles.content}>{children}</View><BottomNavigation active={active} onSelect={onSelect} /></View>; }

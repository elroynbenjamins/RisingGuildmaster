  ...
  return <ScrollView contentContainerStyle={styles.content}>
    ...
    <Panel style={styles.facility}>
      ...
      {facilityUpgrade ? <View style={styles.upgradeBox}><View style={styles.line}><View style={styles.flex}><Text style={styles.panelLabel}>FACILITY UPGRADE ACTIVE</Text><Text style={styles.panelTitle}>Training Hall Level {facilityUpgrade.targetLevel}</Text></View><StatusChip label={`DAY ${facilityUpgrade.completionDay}`} tone="gold" /></View><Text style={styles.meta}>{Math.max(0, facilityUpgrade.completionDay - guild.currentDay)} day(s) remaining</Text><MiniMeter value={facilityProgress} max={facilityDuration} color={colors.green} height={8}/><SecondaryButton iconId="calendar" label="OPEN GUILD CALENDAR" onPress={openCalendar}/></View>
        : upgrade ? <><Pressable accessibilityRole="button" accessibilityLabel="Training hall upgrade plan" accessibilityState={{expanded:showFacilityPlan}} aria-expanded={showFacilityPlan} onPress={()=>setShowFacilityPlan(value=>!value)} style={({pressed})=>[styles.facilityDisclosure,pressed&&styles.pressed]}><View style={styles.flex}><Text style={styles.panelLabel}>NEXT FACILITY LEVEL</Text><Text style={styles.meta}>Level {guild.trainingGround.level + 1} · +1 slot · {upgrade.goldCost.toLocaleString()}G · {upgrade.durationDays} days</Text></View><Text style={styles.facilityDisclosureMark}>{showFacilityPlan?"−":"+"}</Text></Pressable>{showFacilityPlan&&<View style={styles.upgradeBox}><View style={styles.line}><View style={styles.flex}><Text style={styles.panelTitle}>Training Hall Level {guild.trainingGround.level + 1}</Text></View><StatusChip label={`+1 SLOT`} tone="blue" /></View><Text style={styles.meta}>Capacity becomes {TRAINING_GROUND_CONFIG.capacityByLevel[guild.trainingGround.level + 1]} concurrent heroes.</Text>{guild.gold < upgrade.goldCost && <Text style={styles.warning}>Need {(upgrade.goldCost - guild.gold).toLocaleString()} more gold.</Text>}<ActionButton guardMs={600} iconId="training" label={`BEGIN UPGRADE · ${upgrade.goldCost.toLocaleString()}G`} disabled={guild.gold < upgrade.goldCost} onPress={improve}/></View>}</>
    ...
    {catchup && <Panel style={styles.catchup}>
      ...
      {openSideQuests && <ActionButton iconId="quests" label={`BROWSE SIDE QUESTS${catchup.sideQuestCount ? ` · ${catchup.sideQuestCount}` : ""}`} onPress={openSideQuests}/>}
    </Panel>}
...
const styles = StyleSheet.create({
  ...
  levelSmall: { color: colors.muted, fontSize: 10, fontWeight: "800" },
  level: { color: colors.gold, fontSize: 16, fontWeight: "900", marginTop: 2 },
  facilityName: { color: colors.text, fontSize: 16, fontWeight: "900" },
  ...
  facilityDisclosure:{alignItems:"center",backgroundColor:colors.panel2,borderRadius:10,flexDirection:"row",gap:8,minHeight:46,paddingHorizontal:10,paddingVertical:7},
  facilityDisclosureMark:{color:colors.gold,fontSize:20,fontWeight:"900"},
  panelLabel: { color: colors.gold, fontSize: 10, fontWeight: "900", letterSpacing: .8 },
  panelTitle: { color: colors.text, fontSize: 15, fontWeight: "900", marginTop: 2 },
  mastered: { color: colors.green, fontWeight: "900" },
  warning: { color: colors.danger, fontSize: 10, lineHeight: 15 },
  catchup: { borderColor: colors.gold, gap: 8 },
  catchupLabel: { color: colors.gold, fontSize: 10, fontWeight: "900", letterSpacing: .8 },
  catchupTitle: { color: colors.text, fontSize: 17, fontWeight: "900", marginTop: 2 },
  catchupQuest: { color: colors.gold, fontSize: 10, fontWeight: "900", letterSpacing: .5 },
  ...
  emptySlotNumber: { color: colors.muted, fontSize: 10, fontWeight: "900", letterSpacing: .8 },
  emptySlotTitle: { color: colors.green, fontSize: 15, fontWeight: "900", marginTop: 3 },
  session: { borderColor: colors.blue, gap: 8 },
  sessionHead: { alignItems: "center", flexDirection: "row", gap: 9 },
  sessionSlot: { color: colors.blue, fontSize: 10, fontWeight: "900", letterSpacing: .7 },
  sessionName: { color: colors.text, fontSize: 16, fontWeight: "900", marginTop: 2 },
  sessionStats: { flexDirection: "row", flexWrap: "wrap", gap: 10 },
  reward: { color: colors.green, fontSize: 10, fontWeight: "900" },
  heroRow: { gap: 8 },
  hero: { alignItems: "center", minHeight: 142, width: 124 },
  selectedPop:{borderColor:colors.gold,borderWidth:2},selected: { backgroundColor: colors.panel2, borderColor: colors.gold, borderWidth: 1 },
  heroName: { fontSize: 14, lineHeight: 19, textAlign: "center", fontWeight: "900", marginTop: 5, maxWidth: 110 },
  heroLevel: { color: colors.gold, fontSize: 10, fontWeight: "900", marginTop: 3 },
  ...
  programStat: { color: colors.text, fontSize: 10, fontWeight: "800", paddingHorizontal: 7, paddingVertical: 4 },
  programTag: { color: colors.blue, fontSize: 10, fontWeight: "900", paddingHorizontal: 7, paddingVertical: 4 },
  ...
  quoteTitle: { color: colors.text, fontSize: 18, fontWeight: "900", marginTop: 2 },
  ...
  projectionLabel: { color: colors.muted, fontSize: 9, fontWeight: "900", letterSpacing: .7 },
  projectionValue: { color: colors.text, fontSize: 13, fontWeight: "900", marginTop: 3 },
  xpLabel: { color: colors.muted, fontSize: 9, fontWeight: "900", letterSpacing: .5 },
  levelUp: { backgroundColor: colors.panel2, borderColor: colors.green, borderWidth: 1, color: colors.green, fontSize: 10, fontWeight: "900", padding: 8, textAlign: "center" },
  capBox: { backgroundColor: colors.panel2, borderLeftColor: colors.gold, borderLeftWidth: 3, padding: 8 },
  cap: { color: colors.gold, fontSize: 10, fontWeight: "900" },
  ...
  quoteNote: { color: colors.muted, fontSize: 11, lineHeight: 14 }, pressed:{opacity:.76,transform:[{translateY:1}]},
});

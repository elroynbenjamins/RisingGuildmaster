...
export interface UiPreferences { reduceCombatEffects:boolean; reduceMotion:boolean; strongerCombatContrast:boolean; largeText:boolean; tactileFeedback:boolean; confirmEndTurn:boolean; defaultCombatZoom:"fit"|"close"; compactQuestCards:boolean; enemyTurnSpeed:"normal"|"fast"|"very_fast"; themeId?:"guild_dark"|"oled_dark"|"high_contrast" }
export interface GuildState { saveVersion: number; guildId: string; guildName: string; difficultyId: GameDifficultyId; guildmaster: GuildmasterProfile; legacy?: GuildLegacyState; gold: number; gems: number; rations: number; gemTransactions: GemTransaction[]; viewedAdMilestoneDays: number[]; entitlements: ContentEntitlements; dailyLogin: DailyLoginState; finance: GuildFinanceState; reputation: number; heroes: Hero[]; inventory: string[]; materials: MaterialInventory; potions: PotionInventory; artisans: GuildArtisanState; trainingGround: TrainingGroundState; gatheringMissions: GatheringMissionInstance[]; currentDay: number; world: WorldState; recentPartyHeroIds: string[]; partyPresets: PartyPreset[]; equipmentLoadoutsByHeroId: Record<string,EquipmentLoadout[]>; uiPreferences:UiPreferences; discoveredEnemyIds: string[]; questChronicle: QuestChronicleEntry[]; relationships: HeroRelationship[]; recruitment: RecruitmentState; heroContracts: HeroContract[]; huntRewardProgress: Record<string, HuntRewardProgress>; tutorial: TutorialState; rogueliteRotation: RogueliteRotationState; guildOperations: GuildOperationState; raidProgress?: RaidProgressState; unlockedRecipeIds?: string[]; activeRogueliteRun?: RogueliteRunState | null; activeDungeonRun?: DungeonRunState | null }

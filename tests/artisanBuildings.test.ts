...
describe("artisan building progression", () => {
  it.each([
    ["blacksmith", 1, 1100], ["blacksmith", 2, 1900],
    ["tailor", 1, 900], ["tailor", 2, 1600],
    ["jeweler", 1, 1300], ["jeweler", 2, 2200],
  ...
  it("charges gold and completes the Guild Forge after two in-game days", () => {
    ...
    expect(started.gold).toBe(guild.gold - 800);
    expect(started.artisans.blacksmith).toMatchObject({ level: 0, recruited: false, construction: { targetLevel: 1, startDay: 1, completionDay: 3, goldCost: 800 } });
  ...
  it("requires the correct higher-tier Guildmaster node for upgrades", () => {
    ...
    expect(upgrade.gold).toBe(guild.gold - 1100);
  });

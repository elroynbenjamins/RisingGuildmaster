...
describe("Temple services", () => {
  it.each([[100, 90], [11, 10], [1, 1], [0, 0]])("discounts %i missing HP to %i gold", (missingHp, cost) => {
    const base = testHero();
    const hero = { ...base, currentHP: calculateHero(base).stats.maxHP - missingHp };
    expect(getHealingCost(hero)).toBe(cost);
    if (cost > 0) {
      const guild = { ...createGuild(), gold: cost, heroes: [hero] };
      expect(healHero(guild, hero.id).gold).toBe(0);
      expect(fullyTreatHero(guild, hero.id).gold).toBe(0);
      expect(() => healHero({ ...guild, gold: cost - 1 }, hero.id)).toThrow("Not enough gold");
    }
  });

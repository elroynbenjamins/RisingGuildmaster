...
export function CombatScreen({ questId, heroes, combatSetup, initialHeroInstances, onQuestEnd, onEnemiesEncountered, onExit }: { questId: string; heroes: Hero[]; combatSetup?: QuestCombatSetup; initialHeroInstances?: HeroCombatInstance[]; onQuestEnd(status: "victory" | "defeat", instances: HeroCombatInstance[]): void; onEnemiesEncountered?(enemyDefinitionIds: string[]): void; onExit?(): void }) {
  ...
  return <ScrollView contentContainerStyle={styles.content}><View style={styles.titleRow}><Text style={styles.title}>{quest.name}</Text><Pressable onPress={() => { const leave = onExit ?? (() => onQuestEnd("defeat", state.heroes.map((item) => item.instance))); state.status === "active" ? showDialog({ title: "Leave combat?", message: "Current encounter progress will be lost.", eyebrow: "RETREAT WARNING", tone: "danger", actions: [{ label: "Stay", tone: "secondary" }, { label: "Leave", tone: "danger", onPress: leave }] }) : leave(); }}><Text style={styles.exit}>Exit</Text></Pressable></View><Text style={styles.encounter}>Encounter {state.encounterIndex + 1} / {quest.encounterIds.length} · Round {state.round} · Turn {state.turn}</Text><TurnOrder ids={state.turnOrderIds} cursor={state.turnCursor} labels={labels} labelColors={labelColors} aliveIds={aliveIds} />
    ...
    {state.status === "active" && current && <View style={styles.quickSkills}><Text style={styles.quickSkillTitle}>QUICK SKILLS · {current.hero.name}</Text><QuickSkillBar items={skillItems} selectedId={selectedSkillId} onSelect={selectSkill} endControl={<Pressable accessibilityRole="button" onPress={endTurn} style={styles.quickEndButton}><Text style={styles.quickEndText}>END TURN</Text></Pressable>}/><Text style={styles.quickSkillHint}>{selectedSkillId ? `${HERO_SKILLS[selectedSkillId]?.name} queued — double tap an empty teal tile to move, then choose a target from the new position.` : "Tap a skill icon, then choose a highlighted target."}</Text><Text style={styles.reactionStatus}>REACTIONS · {state.heroes.filter((item) => item.unit.isAlive && !state.spentReactionIds.includes(item.unit.combatantId)).length}/{state.heroes.filter((item) => item.unit.isAlive).length} allies ready · Leaving melee reach can trigger an attack.</Text></View>}

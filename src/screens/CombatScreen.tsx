...
export function CombatScreen({ questId, heroes, combatSetup, initialHeroInstances, onQuestEnd, onEnemiesEncountered, onExit }: { questId: string; heroes: Hero[]; combatSetup?: QuestCombatSetup; initialHeroInstances?: HeroCombatInstance[]; onQuestEnd(status: "victory" | "defeat", instances: HeroCombatInstance[]): void; onEnemiesEncountered?(enemyDefinitionIds: string[]): void; onExit?(): void }) {
  ...
  const textScale = guild.uiPreferences.largeText ? 1.15 : 1;
  ...
  const hazardKeys=useMemo(()=>new Set(state.raidMechanic?.telegraphedPositions.map(positionKey)??[]),[state.raidMechanic]);const safeKeys=useMemo(()=>new Set(state.raidMechanic?.safePositions.map(positionKey)??[]),[state.raidMechanic]);const objectiveKeys=useMemo(()=>new Set(state.raidMechanic?.objectives.map((entry)=>positionKey(entry.position))??[]),[state.raidMechanic]);const selectedObjective=selectedPosition?state.raidMechanic?.objectives.find((entry)=>positionKey(entry.position)===positionKey(selectedPosition)):undefined;const canStrikeObjective=Boolean(selectedObjective&&current&&!state.actions.combatActionUsed&&Math.abs(current.unit.position.x-selectedObjective.position.x)+Math.abs(current.unit.position.y-selectedObjective.position.y)<=1);
  ...
  const selectSkill = (id: string) => { lastTargetTap.current = null; setMode("skill"); setSelectedSkillId(id); setInspectedSkillId(id); setSelectedPosition(undefined); setSelectedTargetId(undefined); };
  ...
  if (!state.combatStarted) return <ScrollView contentContainerStyle={styles.content}><View style={styles.titleRow}><Text style={[styles.title,{fontSize:26*textScale}]}>{quest.name}</Text><Pressable onPress={onExit ?? (() => onQuestEnd("defeat", state.heroes.map((item) => item.instance)))}><Text style={styles.exit}>Exit</Text></Pressable></View><Text style={[styles.encounter,{fontSize:12*textScale}]}>Encounter {state.encounterIndex + 1} / {state.encounterIds.length} · {battlefield?.name ?? "Battlefield"}</Text>{state.setupLabel && <Panel><Text style={styles.setup}>{state.setupLabel}</Text></Panel>}<InitiativePreview rolls={state.initiativeRolls} labels={labels} labelColors={labelColors} sides={sides} onBegin={() => setState((value) => beginCombat(value, random.current, 0))} /></ScrollView>;
  return <ScrollView contentContainerStyle={styles.content}><View style={styles.titleRow}><Text style={[styles.title,{fontSize:26*textScale}]}>{quest.name}</Text><Pressable onPress={() => { const leave = onExit ?? (() => onQuestEnd("defeat", state.heroes.map((item) => item.instance))); state.status === "active" ? showDialog({ title: "Leave combat?", message: "Current encounter progress will be lost.", eyebrow: "RETREAT WARNING", tone: "danger", actions: [{ label: "Stay", tone: "secondary" }, { label: "Leave", tone: "danger", onPress: leave }] }) : leave(); }}><Text style={styles.exit}>Exit</Text></Pressable></View><Text style={[styles.encounter,{fontSize:12*textScale}]}>Encounter {state.encounterIndex + 1} / {quest.encounterIds.length} · Round {state.round} · Turn {state.turn}</Text><TurnOrder ids={state.turnOrderIds} cursor={state.turnCursor} labels={labels} labelColors={labelColors} aliveIds={aliveIds} />
    {activeTurnId&&<View accessibilityLiveRegion="polite" style={[styles.turnBanner,activeTurnSide==="enemies"?styles.enemyTurnBanner:styles.heroTurnBanner]}><View style={{alignItems:"center",flexDirection:"row",gap:9}}>{activeEnemy?<EnemyPortrait enemyId={activeEnemy.instance.enemyDefinitionId} size={42}/>:null}<View style={{flex:1}}><Text style={[styles.turnBannerTitle,{fontSize:11*textScale}]}>{activeTurnSide==="enemies"?"ENEMY ACTION":"YOUR TURN"} · {labels[activeTurnId]??"Combatant"}</Text>{latestCombatMessage?<Text numberOfLines={2} style={[styles.turnBannerMessage,{fontSize:10*textScale}]}>{latestCombatMessage}</Text>:null}</View></View></View>}
    <Text style={[styles.encounter,{fontSize:12*textScale}]}>{battlefield?.name ?? "Battlefield"} · {state.board.width}×{state.board.height}</Text><Text style={styles.detail}>{Object.values(battlefield?.legend ?? {}).join("  ·  ")}</Text>{raidPhase&&<Panel style={styles.result}><Text style={styles.victory}>RAID PHASE · {raidPhase.name}</Text><Text style={styles.detail}>{raidPhase.telegraph}</Text><Text style={styles.detail}>{raidPhase.requiredResponse}</Text>{state.raidMechanic?.objectives.length? <Text style={styles.objectiveText}>◆ {state.raidMechanic.objectives.length} raid objective{state.raidMechanic.objectives.length===1?"":"s"} remain · move adjacent and select one</Text>:null}</Panel>}<CombatBoard movementPath={movementPath} board={state.board} units={units} labels={labels} enemyPortraitIds={enemyPortraitIds} effectsByPosition={effectsByPosition} visualEvent={state.lastVisualEvent} reachableKeys={reachableKeys} targetableKeys={targetableKeys} affectedKeys={affectedKeys} hazardKeys={hazardKeys} safeKeys={safeKeys} objectiveKeys={objectiveKeys} selectedUnitId={current?.unit.combatantId} selectedPosition={selectedPosition} onTilePress={onTilePress} />
    {state.status === "active" && current && <View style={styles.quickSkills}><Text style={[styles.quickSkillTitle,{fontSize:10*textScale}]}>QUICK SKILLS · {current.hero.name}</Text><QuickSkillBar items={skillItems} selectedId={selectedSkillId} onSelect={selectSkill} endControl={<Pressable accessibilityRole="button" onPress={endTurn} style={styles.quickEndButton}><Text style={styles.quickEndText}>END TURN</Text></Pressable>}/><Text style={[styles.quickSkillHint,{fontSize:10*textScale}]}>{selectedSkillId ? `${HERO_SKILLS[selectedSkillId]?.name} queued — double tap an empty teal tile to move, then choose a target from the new position.` : "Tap a skill icon, then choose a highlighted target."}</Text><Text style={[styles.reactionStatus,{fontSize:9*textScale}]}>REACTIONS · {state.heroes.filter((item) => item.unit.isAlive && !state.spentReactionIds.includes(item.unit.combatantId)).length}/{state.heroes.filter((item) => item.unit.isAlive).length} allies ready · Leaving melee reach can trigger an attack.</Text></View>}
    <View style={styles.damageLegend}><DamageTypeBadge type="physical"/><Text style={styles.legendText}>vs AC + physical defense</Text><DamageTypeBadge type="magic"/><Text style={styles.legendText}>vs MDS + magic defense</Text><DamageTypeBadge type="true"/><Text style={styles.legendText}>bypasses mitigation</Text></View>
    {inspectedUnit&&<CombatInspectionPanel name={labels[inspectedUnit.combatantId]??"Combatant"} unit={inspectedUnit}/>}
    {selectedObjective&&state.status==="active"&&<Panel style={styles.objectivePanel}><Text style={styles.objectiveTitle}>◆ {selectedObjective.name}</Text><Text style={styles.detail}>Integrity {selectedObjective.currentIntegrity}/{selectedObjective.maxIntegrity}</Text><ActionButton label={canStrikeObjective?"Strike Objective":"Move Adjacent to Strike"} disabled={!canStrikeObjective} onPress={()=>{try{setState((value)=>strikeRaidObjective(value,selectedObjective.position));resetSelection();}catch(caught){setError(caught instanceof Error?caught.message:"Could not strike objective");}}}/></Panel>}
    <View style={styles.actions}><Pressable accessibilityRole="button" onPress={()=>setShowTacticalDetails((value)=>!value)} style={styles.modeButton}><Text style={styles.modeText}>{showTacticalDetails?"Hide Tactical Details":"Potions, Stats & Full Skills"}</Text></Pressable></View>

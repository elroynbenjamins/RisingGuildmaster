...
import { claimDailyLogin } from "../../game/monetization/contentUnlockService";
...
export function GemsSupportScreen({ onBack }: { onBack(): void }) {
  ...
  const removeAds = useCallback(async () => {
    ...
    updateGuild(updated);
    if (activeSaveSlot !== null) await saveGuild(updated, activeSaveSlot);
    setMessage("Thank you for supporting development! Remove Ads is permanently unlocked across your saves. Restore it here with the same Google Play account after reinstalling.");
  }, [updateGuild, activeSaveSlot]);
  ...
  return <ScrollView style={{ backgroundColor: themeColors.background }} contentContainerStyle={styles.content}>
    ...
    <Panel style={styles.gemPanel}><Text style={styles.panelTitle}>Daily Thanks · 10 Gems</Text><Text style={styles.conditions}>A free gift each real-world day. No advertisement or purchase required.</Text><SupportAction primary label={notices.daily ? "Claim 10 Daily Gems" : "Claimed Today"} disabled={!notices.daily} onPress={() => { try { const next = claimDailyLogin(guildRef.current); guildRef.current = next; updateGuild(next); setMessage("Thank you for returning! +10 gems."); } catch (error) { setMessage(error instanceof Error ? error.message : "Already claimed."); } }} /></Panel>
    <Panel style={styles.gemPanel}><Text style={styles.panelTitle}>Remove Ads · Permanent</Text><Text style={styles.conditions}>Remove all ads, including the 20-day milestone ads, across every save. This one-time purchase belongs to your Google Play account and can be restored. Daily login gems remain available; ad-viewing rewards require watching an ad and are not included.</Text><SupportAction primary label={guild.entitlements.adsRemoved ? "Owned · Ads Removed" : gemStore.purchasingProductId === REMOVE_ADS_PRODUCT_ID ? "Processing…" : `Remove Ads · ${adProduct?.displayPrice ?? "Unavailable"}`} disabled={guild.entitlements.adsRemoved || !adProduct || Boolean(gemStore.purchasingProductId)} onPress={() => { void gemStore.purchase(REMOVE_ADS_PRODUCT_ID).catch((error: unknown) => setMessage(error instanceof Error ? error.message : "Purchase unavailable.")); }} /><SupportAction label="Restore Purchases" onPress={() => { void gemStore.retry(); }} /></Panel>

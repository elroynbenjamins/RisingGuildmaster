...
export function calculateWeeklySalary(hero: Hero, archetypeModifier = 0, raceModifier = 0): number { const power = Object.values(hero.baseAttributes).reduce((sum, value) => sum + value, 0); return Math.round((25 + hero.level * 15 + power * .75) * (1 + archetypeModifier + raceModifier + traitModifier(hero, "salary") + getBackgroundModifier(hero.backgroundId, "salary"))); }
